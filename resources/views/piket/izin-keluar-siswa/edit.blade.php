...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .pg-sub { font-size: 13px; color: var(--text4); margin-bottom: 20px; font-family: 'Plus Jakarta Sans', sans-serif; }
...
<div class="pg">
    <a href="{{ route('piket.izin-keluar-siswa.show', $izin) }}" class="back">
        <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
        Kembali ke Detail
    </a>

    <h1 class="pg-title">Edit Izin Keluar Siswa</h1>
    <p class="pg-sub">Hanya izin berstatus <strong>Menunggu</strong> yang dapat diedit</p>

    <div class="alert-warn">
        <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        <span>Setelah izin disetujui atau ditolak, data tidak dapat diubah lagi.</span>
    </div>

    <form action="{{ route('piket.izin-keluar-siswa.update', $izin) }}" method="POST" autocomplete="off">
        @csrf
        @method('PATCH')
                ...
                <div class="fg">
                    <div class="fgroup span2">
                        ...
                        <select name="siswa_id" class="fctl {{ $errors->has('siswa_id') ? 'err' : '' }}" required>
                            ...
                            @foreach($siswas as $s)
                                <option value="{{ $s->id }}"
                                    @selected(old('siswa_id', $izin->siswa_id) == $s->id)>
                    ...
                    <div class="fgroup">
                        ...
                        <select name="tahun_ajaran_id" class="fctl {{ $errors->has('tahun_ajaran_id') ? 'err' : '' }}" required>
                            ...
                            @foreach($tahunAjarans as $ta)
                                <option value="{{ $ta->id }}"
                                    @selected(old('tahun_ajaran_id', $izin->tahun_ajaran_id) == $ta->id)>
                    ...
                    <div class="fgroup">
                        ...
                        <input type="date" name="tanggal"
                            value="{{ old('tanggal', $izin->tanggal->toDateString()) }}"
        ...
        <div class="fc">
                ...
                <div class="fg">
                    ...
                    <div class="fgroup">
                        ...
                        <select name="kategori" class="fctl {{ $errors->has('kategori') ? 'err' : '' }}" required>
                            ...
                            @foreach($kategoriList as $val => $label)
                                <option value="{{ $val }}"
                                    @selected(old('kategori', $izin->kategori) === $val)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="fgroup span2">
                        ...
                        <textarea name="keterangan" rows="3" maxlength="1000"
                            ...
                            class="fctl {{ $errors->has('keterangan') ? 'err' : '' }}">{{ old('keterangan', $izin->keterangan) }}</textarea>
            ...
            <div class="fc-foot">
                ...
                <a href="{{ route('piket.izin-keluar-siswa.show', $izin) }}" class="btn btn-secondary">Batal</a>
            </div>
        </div>
