...
<style>
    ...
    .btn-selesai { background: var(--yellow-bg); color: #a16207; border: 1px solid var(--yellow-border); }
    .btn-selesai:hover { background: #fef9c3; filter: none; }

    /* ── Banner check-in ── */
    .checkin-banner { display: flex; align-items: flex-start; gap: 12px; padding: 12px 18px; background: var(--yellow-bg); border: 1px solid var(--yellow-border); border-radius: var(--radius); margin-bottom: 20px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 13px; font-weight: 600; color: var(--yellow-text); line-height: 1.5; }
    ...
    .badge-dibatalkan{ background: var(--surface3); color: var(--text3); } .badge-dibatalkan .badge-dot { background: var(--text3); }
    ...
    /* Inline selesaikan form */
    .selesai-wrap { display: none; padding: 10px 12px; background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius-sm); margin-top: 8px; }
    .selesai-wrap.open { display: block; }
    .selesai-wrap textarea { width: 100%; min-height: 64px; padding: 8px 10px; border: 1px solid var(--border); border-radius: 6px; font-family: 'DM Sans', sans-serif; font-size: 12.5px; color: var(--text); resize: vertical; outline: none; transition: border-color .15s; box-sizing: border-box; }
    .selesai-wrap textarea:focus { border-color: var(--brand-500); background: #fff; }
    .selesai-actions { display: flex; gap: 6px; margin-top: 8px; justify-content: flex-end; }
...
<div class="page">
        ...
        <form method="GET" action="{{ route('piket.pelanggaran.index') }}">
            <div class="filter-row">
                <input type="text" name="search" class="filter-control"
                    value="{{ request('search') }}" placeholder="Cari nama / NIS siswa…">
                ...
                <input type="date" name="tanggal_dari"   class="filter-control" value="{{ request('tanggal_dari') }}"   max="{{ now()->format('Y-m-d') }}">
                <input type="date" name="tanggal_sampai" class="filter-control" value="{{ request('tanggal_sampai') }}" max="{{ now()->format('Y-m-d') }}">

                <div class="filter-sep"></div>

                @if(request()->hasAny(['search','kelas_id','kategori_id','status','tanggal_dari','tanggal_sampai']))
                    <a href="{{ route('piket.pelanggaran.index') }}" class="btn-reset">
    ...
    <div class="table-card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:600">
                            {{ $p->kategori->nama ?? '—' }}
                        </td>

                        <td class="center">
                            @php $poinClass = $p->poin <= 10 ? 'poin-low' : ($p->poin <= 30 ? 'poin-mid' : 'poin-high'); @endphp
                        ...
                        <td style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;font-size:13px;white-space:nowrap">
                            {{-- FIX: tanggal sudah di-cast sebagai 'date' di model, tidak perlu Carbon::parse --}}
                        ...
                        <td class="center">
                            <div class="action-group">
                                {{-- Detail: semua piket bisa lihat --}}
                                <a href="{{ route('piket.pelanggaran.show', $p->id) }}" class="btn btn-sm btn-detail">Detail</a>

                                {{--
                                    FIX: Strict type-safe ownership check.
                                    $p->dicatat_oleh (int) === $guruAktifId (int|null).
                                    Kedua kondisi harus terpenuhi:
                                    1) Sudah check-in ($guruAktifId tidak null)
                                    2) Adalah pencatat asli
                                --}}
                                @if($guruAktifId && $p->dicatat_oleh === $guruAktifId)
                                    @if($p->status === 'pending')
                                        <a href="{{ route('piket.pelanggaran.edit', $p->id) }}"
                            ...
                            {{-- Inline selesaikan form — hanya owner check-in --}}
                            @if($guruAktifId && $p->dicatat_oleh === $guruAktifId && in_array($p->status, ['pending', 'diproses']))
                            <div id="selesai-{{ $p->id }}" class="selesai-wrap">
                                <form action="{{ route('piket.pelanggaran.selesaikan', $p->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <textarea name="tindakan" maxlength="500"
                                        placeholder="Catatan tindakan (opsional)…">{{ $p->tindakan }}</textarea>
                            ...
                            <div class="empty-state">
                                ...
                                <p class="empty-sub">
                                    @if(request()->hasAny(['search','kelas_id','kategori_id','status','tanggal_dari','tanggal_sampai']))
                                        ...
                                        <a href="{{ route('piket.pelanggaran.index') }}"
        ...
        {{-- ── Pagination (FIX: ellipsis tidak duplikat) ── --}}
        ...
        <div class="pag-wrap">
            ...
            <div class="pag-btns">
                ...
                @foreach($pelanggaran->getUrlRange(1, $last) as $page => $url)
                    @php
                        $show = ($page === 1) || ($page === $last) || (abs($page - $current) <= 1);
                    @endphp
                    @if($show)
