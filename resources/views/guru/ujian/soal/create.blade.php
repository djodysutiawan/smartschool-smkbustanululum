...
/* Alert */
.alert{display:flex;align-items:flex-start;gap:10px;padding:12px 16px;border-radius:var(--radius-sm);margin-bottom:20px;font-size:13px;background:#fee2e2;color:var(--red);border:1px solid #fecaca}
...
.req{color:var(--brand-500);margin-left:2px}
...
.field input:focus,.field select:focus,.field textarea:focus{border-color:var(--brand-500);background:#fff;box-shadow:0 0 0 3px rgba(53,130,240,.08)}
.field input.is-invalid,.field select.is-invalid,.field textarea.is-invalid{border-color:var(--red);background:#fff8f8}
.field-error{font-size:12px;color:var(--red);font-family:'DM Sans',sans-serif;font-weight:600}
.field-hint{font-size:12px;color:var(--text3);font-family:'DM Sans',sans-serif}
.form-row{display:grid;grid-template-columns:1fr 1fr;gap:14px}

/* Jenis soal selector */
.jenis-switch{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
.jenis-opt{display:flex;flex-direction:column;align-items:center;gap:6px;padding:16px 10px;border:2px solid var(--border);border-radius:var(--radius-sm);cursor:pointer;transition:all .15s;text-align:center;position:relative}
.jenis-opt:hover{border-color:var(--border2);background:var(--surface2)}
.jenis-opt.selected{border-color:var(--brand-500);background:var(--brand-50)}
.jenis-opt-icon{width:38px;height:38px;border-radius:9px;background:var(--surface3);display:flex;align-items:center;justify-content:center;transition:background .15s}
.jenis-opt.selected .jenis-opt-icon{background:var(--brand-600)}
.jenis-opt.selected .jenis-opt-icon svg{stroke:#fff}
.jenis-opt-label{font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:700;color:var(--text2)}
.jenis-opt.selected .jenis-opt-label{color:var(--brand-700)}
.jenis-opt-sub{font-size:11.5px;color:var(--text3);line-height:1.4}

/* Pilihan jawaban */
.pilihan-list{display:flex;flex-direction:column;gap:8px}
.pilihan-item{display:flex;align-items:center;gap:10px;padding:10px 12px;border:1.5px solid var(--border);border-radius:var(--radius-sm);background:var(--surface2);transition:border-color .15s,background .15s}
.pilihan-item.is-benar{border-color:#86efac;background:#f0fdf4}
.pilihan-kode{width:30px;height:30px;border-radius:7px;background:var(--surface3);border:1px solid var(--border);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:800;color:var(--text2);display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .15s}
...
.pilihan-radio{width:15px;height:15px;accent-color:var(--green);cursor:pointer;flex-shrink:0}
...
.pilihan-del{background:none;border:none;cursor:pointer;color:var(--text3);padding:4px;display:flex;align-items:center;border-radius:4px;transition:all .15s}
.pilihan-del:hover{color:var(--red);background:#fff0f0}
...
.add-pilihan-btn:hover{border-color:var(--brand-500);color:var(--brand-600);background:var(--brand-50)}

/* Gambar upload */
.file-upload-area{border:2px dashed var(--border2);border-radius:var(--radius-sm);padding:16px;background:var(--surface2);display:flex;align-items:center;gap:12px;cursor:pointer;transition:all .15s}
.file-upload-area:hover{border-color:var(--brand-500);background:var(--brand-50)}
.file-upload-area input[type=file]{display:none}
.file-upload-label{font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:600;color:var(--text2)}
.file-name{font-size:12.5px;color:var(--green);font-family:'DM Sans',sans-serif;font-weight:600;margin-top:4px}
.img-preview{max-width:280px;max-height:180px;border-radius:var(--radius-sm);margin-top:10px;border:1px solid var(--border);display:none}

/* Essay info box */
.info-box{background:var(--brand-50);border:1px solid var(--brand-100);border-radius:var(--radius-sm);padding:14px 16px;font-size:13px;color:var(--brand-700);font-family:'DM Sans',sans-serif;line-height:1.6}
.info-box strong{font-family:'Plus Jakarta Sans',sans-serif;font-weight:700}
...
<div class="page">
    ...
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <a href="{{ route('guru.ujian.show', $ujian) }}">{{ Str::limit($ujian->judul, 30) }}</a>
        <span class="sep">›</span>
        <a href="{{ route('guru.ujian.soal.index', $ujian) }}">Bank Soal</a>
        <span class="sep">›</span>
        <span class="cur">Tambah Soal</span>
    </nav>
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">{{ $ujian->judul }} &middot; Soal ke-{{ $nomorBerikutnya }}</p>
        </div>
        <a href="{{ route('guru.ujian.soal.index', $ujian) }}" class="btn btn-back">
    ...
    <div class="alert">
        ...
        <div>
            <strong style="font-family:'Plus Jakarta Sans',sans-serif">Terdapat {{ $errors->count() }} kesalahan:</strong>
    ...
    <form action="{{ route('guru.ujian.soal.store', $ujian) }}" method="POST"
            ...
            <div class="form-section">
                ...
                <div class="jenis-switch">
                    ...
                    <label class="jenis-opt {{ old('jenis_soal') === 'essay' ? 'selected' : '' }}"
                        ...
                        <p class="jenis-opt-sub">Uraian, koreksi manual</p>
                        <input type="radio" name="jenis_soal" value="essay" style="display:none"
                               {{ old('jenis_soal') === 'essay' ? 'checked' : '' }}>
                    </label>
                </div>
                @error('jenis_soal')<span class="field-error" style="display:block;margin-top:8px">{{ $message }}</span>@enderror
            </div>
        </div>
        ...
        <div class="form-card">
            <div class="form-section">
                ...
                <div class="form-row" style="margin-bottom:14px">
                    <div class="field" style="margin-bottom:0">
                        ...
                        <span class="field-hint">Otomatis ke nomor berikutnya jika dikosongkan</span>
                ...
                <div class="field" style="margin-bottom:0">
                    ...
                    <div class="file-upload-area" onclick="document.getElementById('gambarSoalInput').click()">
                        <svg width="20" height="20" fill="none" stroke="#94a3b8" stroke-width="1.8" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                        <div>
                            <p class="file-upload-label">Klik untuk upload gambar</p>
                            <p class="field-hint" style="margin-top:1px">JPG, PNG, WEBP · Maks 2MB</p>
                        </div>
                        <input type="file" id="gambarSoalInput" name="gambar_soal"
                               accept="image/jpeg,image/png,image/webp"
                               onchange="handleGambar(this,'gambarFileName','gambarPreview')">
                    </div>
                    <p class="file-name" id="gambarFileName" style="display:none"></p>
                    <img id="gambarPreview" class="img-preview" src="#" alt="">
                    @error('gambar_soal')<span class="field-error">{{ $message }}</span>@enderror
                </div>
            </div>

            {{-- Seksi Pilihan (PG & Benar/Salah) --}}
            <div id="seksiPilihan" style="display:none">
                ...
                <div class="form-section">
                    ...
                    <p style="font-size:12.5px;color:var(--text3);margin-bottom:14px;font-family:'DM Sans',sans-serif">
                        Pilih tombol radio di sebelah kanan untuk menandai jawaban yang benar.
                    </p>
            ...
            <div id="seksiEssay" style="display:none">
                ...
                <div class="form-section">
                    <div class="info-box">
                        <strong>Soal Essay</strong> — Jawaban siswa dikoreksi secara manual oleh guru melalui menu
                        <em>Koreksi Essay</em> setelah ujian selesai. Pastikan bobot poin sudah sesuai.
                    </div>
                </div>
            </div>
            ...
            <div class="form-footer">
                ...
                <div class="footer-right">
                    <a href="{{ route('guru.ujian.soal.index', $ujian) }}" class="btn btn-cancel">Batal</a>
...
function addPilihan(kode, teks, locked) {
    ...
    const k   = kode || KODE[idx] || String.fromCharCode(65 + idx);
    const div = document.createElement('div');
    div.className = 'pilihan-item';
    div.id = 'pi_' + idx;
    div.innerHTML = `
        <div class="pilihan-kode" id="kode_${idx}">${k}</div>
        ...
        <input type="radio" name="pilihan_benar_idx" value="${idx}"
               class="pilihan-radio" onchange="markBenar(${idx})">
        <span class="pilihan-benar-lbl" id="blbl_${idx}">Benar</span>
        ${!locked ? `<button type="button" class="pilihan-del" onclick="hapusPilihan(${idx})" title="Hapus pilihan">
...
function markBenar(idx) {
    document.querySelectorAll('.pilihan-item').forEach(el => {
        const id = parseInt(el.id.replace('pi_',''));
        const isB = id === idx;
        el.classList.toggle('is-benar', isB);
        const h = document.getElementById('benar_' + id);
        if (h) h.value = isB ? '1' : '0';
    });
}
...
// ── Gambar preview ──────────────────────────────────────────────────────────
function handleGambar(input, nameId, previewId) {
...
(function () {
    ...
    // Restore old pilihan setelah validation error
    @if(old('jenis_soal') && old('pilihan'))
    document.getElementById('pilihanList').innerHTML = '';
    pilihanCount = 0;
    @php $oldPilihan = old('pilihan', []); @endphp
    @foreach($oldPilihan as $idx => $p)
    addPilihan('{{ $p['kode_pilihan'] }}', '{{ addslashes($p['teks_pilihan'] ?? '') }}', {{ in_array(old('jenis_soal'), ['benar_salah']) ? 'true' : 'false' }});
    @if(($p['adalah_benar'] ?? '0') === '1')
    setTimeout(() => {
        const radio = document.querySelector(`input[name=pilihan_benar_idx][value="{{ $idx }}"]`);
        if (radio) { radio.checked = true; markBenar({{ $idx }}); }
    }, 50);
    @endif
    @endforeach
    @endif
