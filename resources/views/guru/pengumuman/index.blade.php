...
<style>
    ...
    .filter-row input[type=text]{height:36px;padding:0 12px;border:1px solid var(--border);border-radius:var(--radius-sm);font-family:'DM Sans',sans-serif;font-size:13px;color:var(--text);background:var(--surface2);outline:none;transition:border-color .15s;min-width:220px;max-width:100%}
    ...
    /* ── Pin badge ─────────────────────────────────────────────────────────── */
    .pin-badge{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:5px;background:#fef3c7;color:#92400e;font-family:'Plus Jakarta Sans',sans-serif;font-size:10.5px;font-weight:700;vertical-align:middle;margin-left:6px}

    /* ── Two-line cell ─────────────────────────────────────────────────────── */
    .two-line .primary{font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;font-size:13.5px;color:var(--text);display:flex;align-items:center;flex-wrap:wrap;gap:4px}
    .two-line .secondary{font-size:12px;color:var(--text3);margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;max-width:380px}
    ...
    <div class="filter-card">
        <form method="GET" action="{{ route('guru.pengumuman.index') }}">
            <div class="filter-row">
                {{-- FIX: Tambah maxlength sesuai validasi di controller (100 karakter) --}}
    ...
    <div class="table-card">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="two-line">
                                <p class="primary">
                                    {{ $p->judul }}
                                    {{-- Tampilkan pin badge jika dipinned --}}
                                    @if($p->dipinned)
                                        <span class="pin-badge">
                                            <svg width="9" height="9" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3 7h7l-5.5 4 2 7L12 16l-6.5 4 2-7L2 9h7z"/></svg>
                                            Pinned
                                        </span>
                                    @endif
                                </p>
                                @if($p->isi)
                                <p class="secondary">{{ strip_tags($p->isi) }}</p>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="muted" style="font-size:12.5px;white-space:nowrap">
                            {{ $p->dipublikasikan_pada
                                ? $p->dipublikasikan_pada->locale('id')->isoFormat('D MMM Y, HH:mm')
                                : '—' }}
                        </td>
                            ...
                            @if($p->kadaluarsa_pada)
                                @if($p->kadaluarsa_pada->isPast())
                                    <span style="color:#dc2626;font-weight:600">
                                        ...
                                        <br><span style="font-size:11px;font-weight:400">(Kadaluarsa)</span>
                                    </span>
                                @else
                        ...
                        <td class="center">
                            <div class="action-group">
                                <a href="{{ route('guru.pengumuman.show', $p->id) }}"
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                {{-- FIX: Bedakan empty state antara hasil filter vs benar-benar kosong --}}
                                @if(request()->filled('search'))
                                    <p class="empty-title">Tidak ada hasil</p>
                                    <p class="empty-sub">
                                        Tidak ada pengumuman yang cocok dengan "<strong>{{ request('search') }}</strong>".
                                        <a href="{{ route('guru.pengumuman.index') }}" style="color:var(--brand-600)">Tampilkan semua</a>
                                    </p>
                                @else
        ...
        <div class="pag-wrap">
            ...
            <div class="pag-btns">
                ...
                {{-- FIX: Perbaiki ellipsis pagination agar tidak duplikat --}}
                @php
