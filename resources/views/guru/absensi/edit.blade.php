...
<style>
    ...
    .page{padding:28px 28px 48px;max-width:900px}
    ...
    .field input,.field select,.field textarea{padding:9px 12px;border:1px solid var(--border);border-radius:var(--radius-sm);font-family:'DM Sans',sans-serif;font-size:13.5px;color:var(--text);background:var(--surface2);outline:none;transition:border-color .15s,box-shadow .15s;width:100%;box-sizing:border-box}
    .field input:focus,.field select:focus,.field textarea:focus{border-color:var(--brand-500);background:#fff;box-shadow:0 0 0 3px rgba(53,130,240,.1)}
    .field textarea{resize:vertical;min-height:80px}
    .field .error{font-size:11.5px;color:#dc2626;margin-top:3px}

    /* Info readonly */
    .info-readonly{padding:9px 12px;background:var(--surface3);border:1px solid var(--border);border-radius:var(--radius-sm);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:600;color:var(--text2);min-height:40px;display:flex;flex-direction:column;justify-content:center}
    ...
    .status-card{display:flex;flex-direction:column;align-items:center;gap:6px;padding:12px 6px;border:2px solid var(--border);border-radius:var(--radius-sm);cursor:pointer;transition:all .15s;background:var(--surface2);text-align:center;user-select:none}
    .status-card:hover{border-color:var(--border2);background:var(--surface)}
    ...
    /* Upload */
    .existing-file{display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);margin-bottom:10px}
    ...
    .upload-area{border:2px dashed var(--border2);border-radius:var(--radius-sm);padding:22px;text-align:center;background:var(--surface2);cursor:pointer;transition:border-color .15s,background .15s;position:relative}
    ...
    /* Metode icon labels */
    .metode-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
    .metode-option{position:relative}
    .metode-option input[type=radio]{position:absolute;opacity:0;width:0;height:0}
    .metode-card{display:flex;align-items:center;gap:8px;padding:9px 12px;border:2px solid var(--border);border-radius:var(--radius-sm);cursor:pointer;transition:all .15s;background:var(--surface2);user-select:none}
    .metode-card:hover{border-color:var(--border2);background:var(--surface)}
    .metode-option input:checked + .metode-card{border-color:var(--brand-500);background:var(--brand-50);box-shadow:0 0 0 3px var(--brand-100)}
    .metode-dot{width:8px;height:8px;border-radius:50%;background:var(--text3);flex-shrink:0}
    .metode-option input:checked + .metode-card .metode-dot{background:var(--brand-600)}
    .metode-label{font-family:'Plus Jakarta Sans',sans-serif;font-size:12.5px;font-weight:700;color:var(--text2)}
    .metode-option input:checked + .metode-card .metode-label{color:var(--brand-700)}
...
<div class="page">
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">{{ $absensi->siswa->nama_lengkap ?? '—' }} — {{ \Carbon\Carbon::parse($absensi->tanggal)->locale('id')->isoFormat('D MMMM Y') }}</p>
        </div>
    ...
    <form action="{{ route('guru.absensi.update', $absensi->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-card-body grid-2">
                <div class="field">
                    ...
                    <div class="info-readonly">
                        ...
                        <small>NIS: {{ $absensi->siswa->nis ?? '—' }}</small>
                    </div>
                </div>
                <div class="field">
                    <label>Kelas</label>
                ...
                <div class="field">
                    <label>Tanggal</label>
                    <div class="info-readonly">{{ \Carbon\Carbon::parse($absensi->tanggal)->locale('id')->isoFormat('D MMMM Y') }}</div>
                </div>
                <div class="field">
        ...
        <div class="form-card">
            ...
            <div class="form-card-body">
                <div class="status-grid">
                    ...
                    <label class="status-option">
                        <input type="radio" name="status" value="{{ $s }}"
                               {{ old('status', $absensi->status) === $s ? 'checked' : '' }}>
                ...
                @error('status')
                    <span class="error" style="margin-top:4px;display:block">{{ $message }}</span>
                @enderror
            </div>
        </div>
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <span class="form-card-title">Detail Waktu</span>
            </div>
            <div class="form-card-body grid-2">
                <div class="field">
                ...
                <div class="field span-2">
                    ...
                    <textarea name="keterangan" placeholder="Catatan tambahan…" rows="3">{{ old('keterangan', $absensi->keterangan) }}</textarea>
        ...
        <div class="form-card">
            <div class="form-card-header">
                <svg width="14" height="14" fill="none" stroke="var(--brand-600)" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                <span class="form-card-title">Metode Absensi <span style="font-weight:400;color:var(--text3);font-size:12px">(opsional)</span></span>
            </div>
            <div class="form-card-body">
                {{-- FIX: $metodeList sekarang dikirim dari controller --}}
                <div class="metode-grid">
                    ...
                    <label class="metode-option">
                        <input type="radio" name="metode" value="{{ $m }}"
                               {{ old('metode', $absensi->metode) === $m ? 'checked' : '' }}>
                        <div class="metode-card">
                            <span class="metode-dot"></span>
                            <span class="metode-label">{{ $metodeLabels[$m] ?? ucfirst($m) }}</span>
                        </div>
                    </label>
                    @endforeach
                </div>
                @error('metode')<span class="error" style="margin-top:4px;display:block">{{ $message }}</span>@enderror
            </div>
        </div>
            ...
            <div class="form-card-body">
                ...
                <div class="existing-file">
                    ...
                    <div style="flex:1;overflow:hidden">
                        <p style="font-family:'Plus Jakarta Sans',sans-serif;font-size:12.5px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                    ...
                    <a href="{{ Storage::url($absensi->path_surat_izin) }}" target="_blank"
                ...
                <div class="upload-area" id="uploadArea">
                    <input type="file" name="path_surat_izin" id="suratInput"
                           accept=".pdf,.jpg,.jpeg,.png"
                           onchange="showFile(this,'suratLabel')">
                    ...
                    <p class="upload-label">{{ $absensi->path_surat_izin ? 'Klik untuk ganti file' : 'Klik untuk pilih file' }}</p>
                    <p class="upload-hint">PDF, JPG, PNG — maksimal 2MB</p>
                    <p id="suratLabel" class="upload-filename"></p>
                </div>
                @error('path_surat_izin')
                    <span class="error" style="margin-top:4px;display:block">{{ $message }}</span>
                @enderror
            </div>
        </div>
...
function showFile(input, labelId) {
    ...
    if (input.files && input.files[0]) {
        ...
        document.getElementById('uploadArea').style.borderColor = '#1f63db';
        document.getElementById('uploadArea').style.background  = '#eef6ff';
    }
}
</script>
</x-app-layout>