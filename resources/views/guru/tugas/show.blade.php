{{-- resources/views/guru/tugas/show.blade.php --}}
<x-app-layout>
<style>
    ...
    .detail-card-title{font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px}
...
<form action="{{ route('guru.tugas.destroy', $tugas->id) }}"
      method="POST" id="delForm" style="display:none">
    @csrf
    @method('DELETE')
</form>
...
<div class="page">
    <div class="page-header">
        ...
        <div class="header-actions">
            ...
            <button type="button" class="btn btn-del" id="btnHapusHeader"
    ...
    {{--
        FIX ParseError: Pisahkan semua @php variable assignment ke satu blok
        di atas, tidak di-inline di tengah HTML. Ini mencegah syntax error
        akibat Blade melakukan parsing yang ambigu pada ternary multi-baris
        di dalam @php yang berdekatan dengan directive @if/@endif.
    --}}
    @php
        $batasWaktu = \Carbon\Carbon::parse($tugas->batas_waktu);
        $expired    = $batasWaktu->isPast();
        $pct        = $stats['total_siswa'] > 0
            ? round(($stats['terkumpul'] / $stats['total_siswa']) * 100)
            : 0;
        $pctNilai   = $stats['terkumpul'] > 0
            ? round(($stats['sudah_dinilai'] / $stats['terkumpul']) * 100)
            : 0;
        $adaUpdatedAt = $tugas->updated_at && $tugas->created_at
            && $tugas->updated_at->ne($tugas->created_at);
    @endphp
    ...
    <div class="layout">
        <div>
                ...
                <div class="detail-card-body">
                    <div class="info-grid">
                        <div class="info-row">
                            ...
                            <span class="info-val">{{ $tugas->kelas->nama_kelas ?? '—' }}</span>
                        </div>
                        <div class="info-row">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-val">{{ $tugas->tahunAjaran?->tahun ?? '—' }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Jenis Pengumpulan</span>
                            <span class="info-val">{{ strtoupper($tugas->jenis_pengumpulan) }}</span>
                        </div>
                        <div class="info-row">
                            ...
                            <span class="info-val {{ $expired ? 'deadline-red' : '' }}">
                                ...
                                @if($expired)
                                    <span class="badge badge-expired" style="margin-left:6px">Kedaluwarsa</span>
                                @endif
                            </span>
                        </div>
                        <div class="info-row">
                        ...
                        <div class="info-row">
                            <span class="info-label">Izin Terlambat</span>
                            <span class="info-val">{{ $tugas->izinkan_terlambat ? 'Ya' : 'Tidak' }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Dibuat</span>
                            <span class="info-val">{{ $tugas->created_at->format('d M Y, H:i') }}</span>
                        </div>
                        {{--
                            FIX ParseError: Pindahkan kondisi pengecekan ke variabel $adaUpdatedAt
                            yang sudah dihitung di blok @php di atas. Menaruh ekspresi Carbon
                            ->ne() langsung di @if bisa menyebabkan Blade parser gagal
                            mem-parse karakter -> sebagai token yang valid di versi tertentu.
                        --}}
                        @if($adaUpdatedAt)
                        <div class="info-row">
                            <span class="info-label">Terakhir Diperbarui</span>
                            <span class="info-val">{{ $tugas->updated_at->format('d M Y, H:i') }}</span>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="detail-card">
                ...
                <div class="detail-card-body">
                    <p class="desc-box">{{ $tugas->deskripsi }}</p>
                </div>
            </div>
            @endif
            ...
            <div class="detail-card">
                <div class="detail-card-header">
                    <span class="detail-card-title">
                        ...
                        Riwayat Pengumpulan ({{ $tugas->pengumpulan->count() }})
                    </span>
                    <a href="{{ route('guru.pengumpulan-tugas.index', ['tugas_id' => $tugas->id]) }}"
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="center">
                                    @php
                                        $nilaiFloat = $p->nilai !== null ? (float) $p->nilai : null;
                                        $nc = 'nilai-none';
                                        if ($nilaiFloat !== null) {
                                            $nc = $nilaiFloat >= 75 ? 'nilai-high'
                                                : ($nilaiFloat >= 50 ? 'nilai-mid' : 'nilai-low');
                                        }
                                    @endphp
                                    <span class="nilai-chip {{ $nc }}">
                                        {{ $nilaiFloat !== null ? $nilaiFloat : '—' }}
                                    </span>
                                </td>
                                <td class="center">
                                    <div class="action-group">
                                        ...
                                        @if(in_array($p->status, ['dikumpulkan', 'terlambat', 'sudah_dinilai']))
                                            <button type="button" class="btn btn-sm btn-nilai"
                                                data-id="{{ $p->id }}"
                                                data-maks="{{ $tugas->nilai_maksimal ?? 100 }}"
                                                data-nilai="{{ $p->nilai ?? '' }}"
                                                data-umpan="{{ $p->umpan_balik ?? '' }}"
                                                data-route="{{ route('guru.pengumpulan-tugas.beri-nilai', $p->id) }}"
                                                onclick="openNilai(this)">
                                                Nilai
                                            </button>
                                        @endif
                                    </div>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" style="text-align:center;padding:32px;color:var(--text3);font-size:13px">
...
function confirmDelete() {
    const judul = document.getElementById('btnHapusHeader').dataset.judul;
    Swal.fire({
        title: 'Hapus Tugas?',
        html: 'Tugas <strong>' + judul + '</strong> dan semua pengumpulan siswa akan dihapus permanen.',
        icon: 'warning',
...
function openNilai(btn) {
    document.getElementById('nilaiForm').action  = btn.dataset.route;
    document.getElementById('nilaiInput').max    = btn.dataset.maks;
    document.getElementById('nilaiInput').value  = btn.dataset.nilai;
    document.getElementById('umpanBalikInput').value = btn.dataset.umpan;
    document.getElementById('nilaiHint').textContent  = 'maks. ' + btn.dataset.maks;
...
document.getElementById('nilaiModal').addEventListener('click', e => {
    if (e.target === document.getElementById('nilaiModal')) closeNilai();
});
</script>
</x-app-layout>