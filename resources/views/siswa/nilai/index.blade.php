...
<div class="page">
    ...
    @php
        ...
        $rataFormatted = ! is_null($rataRataAkhir)
            ? number_format((float) $rataRataAkhir, 1)
            : '—';
    @endphp
            ...
            <div class="filter-row">
                ...
                <select name="mapel_id">
                    ...
                    @foreach($mapelList as $m)
                        <option value="{{ $m->id }}" {{ (string) request('mapel_id') === (string) $m->id ? 'selected' : '' }}>
                            {{ $m->nama_mapel }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    ...
                    @php
                        /*
                         * SOLUSI FINAL — tidak menggunakan closure $nilaiClass.
                         *
                         * Blade Component (<x-app-layout>) mengkompilasi default slot
                         * sebagai PHP closure. Variable yang didefinisikan via @php
                         * di LUAR forelse tidak masuk ke scope closure tersebut,
                         * sehingga selalu menghasilkan "Undefined variable $nilaiClass".
                         *
                         * Solusi: gunakan PHP named function (function_exists guard)
                         * yang didefinisikan di dalam @php block pertama kali dieksekusi,
                         * lalu panggil per iterasi. Named function bersifat global di
                         * request PHP sehingga function_exists guard memastikan tidak
                         * terjadi redeclaration error.
                         */
                        if (! function_exists('nilaiPillClass')) {
                            function nilaiPillClass(?float $v): string {
                        ...
                        $cls_tugas  = nilaiPillClass($n->nilai_tugas  !== null ? (float) $n->nilai_tugas  : null);
                        $cls_harian = nilaiPillClass($n->nilai_harian !== null ? (float) $n->nilai_harian : null);
                        $cls_uts    = nilaiPillClass($n->nilai_uts    !== null ? (float) $n->nilai_uts    : null);
                        $cls_uas    = nilaiPillClass($n->nilai_uas    !== null ? (float) $n->nilai_uas    : null);
                        $cls_akhir  = nilaiPillClass($n->nilai_akhir  !== null ? (float) $n->nilai_akhir  : null);
                    @endphp
                    ...
                    <tr>
                        ...
                        <td class="center">
                            <span class="nilai-pill {{ $cls_tugas }}">
                                {{ $n->nilai_tugas !== null ? number_format((float) $n->nilai_tugas, 0) : '—' }}
                            </span>
                        </td>

                        <td class="center">
                            <span class="nilai-pill {{ $cls_harian }}">
                                {{ $n->nilai_harian !== null ? number_format((float) $n->nilai_harian, 0) : '—' }}
                            </span>
                        </td>

                        <td class="center">
                            <span class="nilai-pill {{ $cls_uts }}">
                                {{ $n->nilai_uts !== null ? number_format((float) $n->nilai_uts, 0) : '—' }}
                            </span>
                        </td>

                        <td class="center">
                            <span class="nilai-pill {{ $cls_uas }}">
                                {{ $n->nilai_uas !== null ? number_format((float) $n->nilai_uas, 0) : '—' }}
                            </span>
                        </td>

                        <td class="center">
                            <span class="nilai-pill {{ $cls_akhir }}" style="font-size:14px;min-width:50px">
                                {{ $n->nilai_akhir !== null ? number_format((float) $n->nilai_akhir, 1) : '—' }}
                            </span>
                        </td>

                        {{-- predikat null → class "pred-" → CSS abu-abu --}}
                        <td class="center">
                            <span class="pred-badge pred-{{ $n->predikat ?? '' }}">
                                {{ $n->predikat ?? '—' }}
                            </span>
                        </td>
