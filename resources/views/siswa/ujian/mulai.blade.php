...
<style>
    ...
    /* FIX [Minor]: @keyframes harus berada di dalam <style>, bukan di dalam <script> */
...
<div class="page">
    ...
    <div class="ujian-hero">
        ...
        @php $jenisLabel = ['ulangan_harian'=>'Ulangan Harian','uts'=>'UTS','uas'=>'UAS','kuis'=>'Kuis','quiz'=>'Quiz','remedial'=>'Remedial'][$ujian->jenis] ?? ucfirst($ujian->jenis); @endphp
    ...
    @if($percobaanKe > 0)
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;gap:14px">
        <div style="flex:1">
            <p style="font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px">Riwayat Percobaan</p>
            <p style="font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:700;color:var(--text)">
                Percobaan ke-{{ $percobaanKe }} dari {{ $ujian->maks_percobaan ?? 1 }}
                @if($nilaiTertinggi !== null)
                    · Nilai tertinggi: <span style="color:var(--brand)">{{ number_format($nilaiTertinggi, 1) }}</span>
                @endif
            </p>
        </div>
    </div>
    ...
    <div class="action-area">
        ...
        @if($sesiAktif)
            {{--
                FIX [Critical]: Sebelumnya menggunakan $sesiAktif->id sebagai parameter,
                padahal route siswa.ujian.kerjakan mengharapkan {ujian} (ID ujian),
                bukan ID sesi. Diperbaiki menjadi $ujian->id.
            --}}
        ...
        @else
            {{--
                FIX [Critical]: Form action sebelumnya mengarah ke route 'siswa.ujian.mulai'
                yang merupakan GET route (halaman konfirmasi ini sendiri). Harus mengarah ke
                'siswa.ujian.start' yang merupakan POST route untuk membuat sesi baru.
            --}}
    ...
    function konfirmasiMulai() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                btn.disabled = true;
                // FIX [Minor]: @keyframes spin dipindah ke <style> di atas.
                // Di sini cukup referensikan nama animasi yang sudah terdaftar.
