...
<style>
    ...
    *{box-sizing:border-box;}
    ...
    {{--
        Charts guru: polar area (status) + doughnut (jenis kelamin)
        Beda dari: absensi=line+doughnut, siswa=bar+pie, pelanggaran=line+doughnut merah
    --}}
    ...
    .avatar{width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:800;flex-shrink:0;background:var(--brand-50);color:var(--brand-600);}
    ...
    .akun-dot.on{background:#22c55e;} .akun-dot.off{background:#cbd5e1;}
...
<div class="page">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Data seluruh guru — filter, rekap kepegawaian, dan ekspor laporan</p>
        </div>
    ...
    {{--
        ══ STAT CARDS ══
        Sumber: $statsG dari controller — ['total', 'aktif', 'laki', 'perempuan']
        Semua via GROUP BY di controller, tidak ada query di view.
    --}}
    ...
    {{--
        ══ CHARTS ══
        Guru: polar area (aktif vs non) + doughnut jenis kelamin.
        Polar area jarang dipakai di halaman lain — membuat halaman ini distinctif.
        FIX lama: bar chart hardcode 0 untuk cuti & pensiun — sekarang pakai data real
        dari $statsG yang tersedia (aktif vs total-aktif), atau tambahkan breakdown
        di controller jika perlu detail cuti/pensiun.
    --}}
    <div class="charts-row">
        <div class="card">
            <div class="card-header">
                <span class="card-title">Status Kepegawaian Guru</span>
                <span class="card-sub">Aktif vs Non-Aktif (data global)</span>
            </div>
            <div class="card-body">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <span class="card-sub">Guru aktif</span>
            </div>
            <div class="card-body">
    ...
    {{--
        ══ FILTER ══
        Sesuai applyGuruFilters(): status, jenis_kelamin, status_kepegawaian, search
        FIX: tambah status_kepegawaian yang ada di controller tapi tidak di view lama.
    --}}
    <div class="filter-card">
        <form method="GET" action="{{ route('admin.laporan.guru') }}">
            <div class="filter-grid">
                ...
                {{-- FIX: status_kepegawaian ada di applyGuruFilters() tapi hilang di view lama --}}
                <div class="field">
    ...
    <div class="table-card">
        <div class="table-topbar">
            <p class="table-info">
                Data Guru
                @if($guru->total() > 0)
                    <span>— {{ $guru->firstItem() }}–{{ $guru->lastItem() }} dari {{ number_format($guru->total()) }} record</span>
                @else
                    <span>— Tidak ada data</span>
                @endif
            </p>
            <div class="table-actions">
                <a href="{{ route('admin.laporan.guru.export.pdf', request()->query()) }}"
                ...
                <a href="{{ route('admin.laporan.guru.export.excel', request()->query()) }}"
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;align-items:center;gap:10px;">
                                ...
                                <div>
                                    ...
                                    @if($g->email ?? false)
                                        <p style="font-size:11.5px;color:var(--text3);font-family:'DM Sans',sans-serif;">{{ $g->email }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- Status Kepegawaian --}}
                        <td class="muted">{{ $g->status_kepegawaian ?? '—' }}</td>

                        {{--
                            Akun — relasi pengguna() via pengguna_id.
                            FIX: cek $g->pengguna dengan optional() agar tidak fatal.
                        --}}
                        <td>
                            @if(optional($g->pengguna)->id)
                                <div class="akun-pill">
                        ...
                        {{--
                            Status — guard CSS agar tidak error jika nilai di luar daftar valid.
                            FIX: class badge-nonaktif tersedia tapi CSS badge-tidak_aktif tidak — samakan.
                        --}}
                        <td>
                            @php
                                $validSt = ['aktif', 'nonaktif', 'cuti', 'pensiun'];
                                $st = in_array($g->status, $validSt) ? $g->status : 'aktif';
                                $stLabel = ['aktif' => 'Aktif', 'nonaktif' => 'Non-Aktif', 'cuti' => 'Cuti', 'pensiun' => 'Pensiun'];
                            @endphp
                            <span class="badge badge-{{ $st }}">
                                <span class="badge-dot"></span>{{ $stLabel[$st] ?? ucfirst($st) }}
                            </span>
                        </td>
        ...
        @if($guru->hasPages())
        <div class="pag-wrap">
            <p class="pag-info">Menampilkan {{ $guru->firstItem() }} – {{ $guru->lastItem() }} dari {{ number_format($guru->total()) }} data</p>
            <div class="pag-btns">
                @if($guru->onFirstPage())
                ...
                @php $cur = $guru->currentPage(); $last = $guru->lastPage(); $ellL = false; $ellR = false; @endphp
                @foreach($guru->getUrlRange(1, $last) as $page => $url)
...
<script>
    ...
    {{--
        Chart 1: Polar area — status guru (aktif vs non-aktif).
        Polar area = beda dari semua halaman lain. Distinctive untuk halaman guru.
        FIX lama: bar chart hardcode 0 untuk cuti & pensiun.
        Sekarang: hanya pakai data yang memang ada di $statsG (aktif & non-aktif).
        Jika controller nanti kirim breakdown cuti/pensiun, tambahkan di sini.
    --}}
    new Chart(document.getElementById('statusChart'), {
        type: 'polarArea',
        data: {
            labels: ['Guru Aktif', 'Non-Aktif / Cuti / Pensiun'],
            datasets: [{
                data: [
                    {{ $statsG['aktif'] }},
                    {{ max(0, $statsG['total'] - $statsG['aktif']) }},
                ],
                backgroundColor: ['rgba(34,197,94,.65)', 'rgba(148,163,184,.4)'],
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${ctx.raw} guru` } }
            },
            scales: {
    ...
    {{--
        Chart 2: Doughnut — jenis kelamin guru.
        Sama tipe dengan pelanggaran/absensi tapi warna beda (biru langit & pink),
        posisi legend di bawah (bukan kanan).
        Data dari $statsG['laki'] & $statsG['perempuan'] — via GROUP BY di controller.
    --}}
