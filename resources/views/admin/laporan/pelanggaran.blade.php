...
<div class="page">
    ...
    {{-- Stats Strip — data dari $statsP yang di-pass controller --}}
    ...
    {{--
        Chart — gunakan $statsP dari controller (sudah dihitung via GROUP BY).
        Pembagi untuk chart tingkat: $statsP['total'] (total semua pelanggaran di DB).
        Pembagi untuk chart status:  $statsP['total'].
        Tidak ada query inline di view agar tidak terjadi N+1.
    --}}
    @php
        $totalForChart = max($statsP['total'], 1);

        // Data tingkat diambil dari $kategoriList yang sudah di-pass controller.
        // Hitung per tingkat dari koleksi $kategoriList agar tidak query baru.
        // Controller pass $kategoriList = KategoriPelanggaran::orderBy('nama')->get()
        // tapi tidak pass jumlah per tingkat — kita hitung dari $pelanggaran (halaman saat ini)
        // TIDAK IDEAL untuk chart global. Solusi terbaik: hitung di controller.
        // Di sini kita gunakan pendekatan yang aman: hitung dari data yang sudah ada
        // yaitu menggunakan total dari statsP untuk pembagi, dan query sekali via eager-loaded.
        // Karena controller TIDAK mengirim data tingkat, kita buat query ringan di sini
        // dengan cara yang benar (satu query GROUP BY, bukan 3 query terpisah).
        $tingkatCounts = \App\Models\KategoriPelanggaran::selectRaw('tingkat, COUNT(*) as jumlah')
            ->join('pelanggarans', 'kategori_pelanggarans.id', '=', 'pelanggarans.kategori_pelanggaran_id')
            ->groupBy('tingkat')
            ->pluck('jumlah', 'tingkat')
            ->toArray();
        $beratCount  = $tingkatCounts['berat']  ?? 0;
        $sedangCount = $tingkatCounts['sedang'] ?? 0;
        $ringanCount = $tingkatCounts['ringan'] ?? 0;
    @endphp
    ...
    <div class="chart-grid">
        <div class="chart-card">
            ...
            <div class="bar-list">
                @foreach([
                    ['Berat',  $beratCount,  'red'],
                    ['Sedang', $sedangCount, 'orange'],
                    ['Ringan', $ringanCount, 'yellow'],
                ] as [$label, $val, $color])
                <div>
                    <div class="bar-item-label"><span>{{ $label }}</span><span style="font-weight:700">{{ $val }}</span></div>
                    <div class="bar-track"><div class="bar-fill {{ $color }}" style="width:{{ $totalForChart > 0 ? round($val / $totalForChart * 100) : 0 }}%"></div></div>
                </div>
                @endforeach
            </div>
        </div>
        <div class="chart-card">
            ...
            <div class="bar-list">
                @foreach([
                    ['Diproses', $statsP['diproses'], 'blue'],
                    ['Selesai',  $statsP['selesai'],  'green'],
                    ['Banding',  $statsP['banding'],  'yellow'],
                ] as [$label, $val, $color])
                <div>
                    <div class="bar-item-label"><span>{{ $label }}</span><span style="font-weight:700">{{ $val }}</span></div>
                    <div class="bar-track"><div class="bar-fill {{ $color }}" style="width:{{ $totalForChart > 0 ? round($val / $totalForChart * 100) : 0 }}%"></div></div>
    ...
    {{-- Filter — sesuaikan dengan semua parameter yang didukung applyPelanggaranFilters() --}}
    <div class="filter-card">
        <form method="GET" action="{{ route('admin.laporan.pelanggaran') }}">
            <div class="filter-row">
                {{-- search: whereHas siswa nama_lengkap / nis --}}
                <input type="text" name="search" placeholder="Cari nama / NIS siswa…" value="{{ request('search') }}">
                ...
                {{-- kelas_id: whereHas siswa → kelas_id — gunakan $kelasList dari controller --}}
                ...
                {{-- kategori_id: where('kategori_pelanggaran_id') --}}
                ...
                {{-- tanggal range: whereDate tanggal --}}
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            {{-- Pastikan badge class ada untuk semua nilai status yang mungkin --}}
                            <span class="badge badge-{{ in_array($p->status, ['pending','diproses','selesai','banding']) ? $p->status : 'pending' }}">
