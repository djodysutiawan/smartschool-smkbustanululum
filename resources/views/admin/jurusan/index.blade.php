...
<div class="page">
    ...
    {{--
        FIX: Stats sekarang pakai variabel $stats dari controller.
        Sebelumnya memanggil \App\Models\Jurusan::where(...)->count() langsung
        di view — logika bisnis tidak boleh ada di blade template.
    --}}
    <div class="stats-strip">
        <div class="stat-card">
            ...
            <div><p class="stat-label">Total Jurusan</p><p class="stat-val">{{ $stats['total'] }}</p></div>
        </div>
        <div class="stat-card">
            ...
            <div><p class="stat-label">Published</p><p class="stat-val">{{ $stats['published'] }}</p></div>
        </div>
        <div class="stat-card">
            ...
            <div><p class="stat-label">Draft</p><p class="stat-val">{{ $stats['draft'] }}</p></div>
        </div>
        <div class="stat-card">
            ...
            <div><p class="stat-label">Penerimaan Buka</p><p class="stat-val">{{ $stats['penerimaan_buka'] }}</p></div>
        </div>
    </div>
    ...
    <div class="table-card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="center">
                            {{-- withCount sudah di-load dari controller, tidak ada N+1 --}}
                            <div class="mini-stats">
                                <div class="mini-stat">
                                    <span class="mini-stat-val">{{ $j->kurikulum_count }}</span>
                                    <span class="mini-stat-label">Mapel</span>
                                </div>
                                <div class="mini-stat">
                                    <span class="mini-stat-val">{{ $j->kompetensi_count }}</span>
                                    <span class="mini-stat-label">Kompetensi</span>
                                </div>
                                <div class="mini-stat">
                                    <span class="mini-stat-val">{{ $j->fasilitas_count }}</span>
                                    <span class="mini-stat-label">Fasilitas</span>
                                </div>
                            </div>
                        </td>
                        <td class="center">
                            ...
                            <div class="action-group">
                                ...
                                <form action="{{ route('admin.jurusan.destroy', $j->id) }}" method="POST" id="delForm-{{ $j->id }}">
                                    ...
                                    <button type="button" class="btn btn-sm btn-del"
                                        onclick="confirmDelete(document.getElementById('delForm-{{ $j->id }}'), '{{ addslashes($j->nama) }}')">
                                        Hapus
                                    </button>
                                </form>
                            </div>
                    ...
                    <tr><td colspan="8">
                        <div class="empty-state">
                            <div class="empty-icon">
                                <svg width="24" height="24" fill="none" stroke="#94a3b8" stroke-width="1.8" viewBox="0 0 24 24"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/></svg>
                            </div>
        ...
        <div class="pag-wrap">
            ...
            <div class="pag-btns">
                ...
                @else
                    <a href="{{ $jurusan->previousPageUrl() }}" class="pag-btn">
                        <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
                    </a>
                @endif
                ...
                @if($jurusan->hasMorePages())
                    <a href="{{ $jurusan->nextPageUrl() }}" class="pag-btn">
...
function confirmDelete(form, nama) {
    Swal.fire({
        title: 'Hapus Jurusan?',
        text: `Jurusan "${nama}" akan dihapus permanen beserta semua datanya.`,
        icon: 'warning',
