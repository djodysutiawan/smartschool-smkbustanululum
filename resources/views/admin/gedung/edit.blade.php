...
<div class="page">
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <a href="{{ route('admin.gedung.show', $gedung->id) }}">{{ $gedung->nama_gedung }}</a>
        <span class="sep">›</span>
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Perbarui data {{ $gedung->nama_gedung }}</p>
        </div>
        <a href="{{ route('admin.gedung.show', $gedung->id) }}" class="btn btn-back">
            <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
            Kembali
        </a>
    </div>

    <form action="{{ route('admin.gedung.update', $gedung->id) }}" method="POST" id="gedungForm">
        @csrf @method('PUT')
        <div class="form-card">
                ...
                <div class="form-grid">
                    <div class="field">
                        ...
                        <input type="text" name="nama_gedung" value="{{ old('nama_gedung', $gedung->nama_gedung) }}"
                            ...
                            class="{{ $errors->has('nama_gedung') ? 'is-invalid' : '' }}">
                        @error('nama_gedung')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Kode Gedung <span class="req">*</span></label>
                        <input type="text" name="kode_gedung" value="{{ old('kode_gedung', $gedung->kode_gedung) }}"
                            placeholder="cth. GDG-A" maxlength="10"
                            class="{{ $errors->has('kode_gedung') ? 'is-invalid' : '' }}">
                        @error('kode_gedung')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Jumlah Lantai <span class="req">*</span></label>
                        <input type="number" name="jumlah_lantai" value="{{ old('jumlah_lantai', $gedung->jumlah_lantai) }}"
                            min="1" max="20"
                            class="{{ $errors->has('jumlah_lantai') ? 'is-invalid' : '' }}">
                        <span class="field-hint">Jumlah lantai gedung (1–20)</span>
                        @error('jumlah_lantai')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    ...
                    <div class="field col-span-2">
                        ...
                        <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat tentang gedung ini (opsional)..."
                            class="{{ $errors->has('deskripsi') ? 'is-invalid' : '' }}">{{ old('deskripsi', $gedung->deskripsi) }}</textarea>
            ...
            <div class="form-footer">
                <a href="{{ route('admin.gedung.show', $gedung->id) }}" class="btn btn-cancel">Batal</a>
...
<script>
    ...
    document.getElementById('gedungForm').addEventListener('submit', function () {
