...
<style>
    ...
    *{box-sizing:border-box;}
    ...
    .cascade-loading{font-size:11.5px;color:var(--brand);font-family:'DM Sans',sans-serif;margin-top:-2px;display:none;}
...
<div class="page">
    <nav class="breadcrumb">
        {{-- FIX: route names disesuaikan dengan web.php --}}
        ...
        <div class="form-card">
                ...
                <div class="form-grid">
                    ...
                    <div class="field col-span-2">
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}"
                               placeholder="cth. Ulangan Harian Bab 3 — Aljabar"
                               class="{{ $errors->has('judul') ? 'is-invalid' : '' }}">
                        @error('judul')<span class="field-error">{{ $message }}</span>@enderror
                    </div>

                    {{-- Guru (harus dipilih duluan agar cascade berjalan) --}}
                    <div class="field">
                    ...
                    <div class="field">
                        ...
                        <select name="tahun_ajaran_id"
                            ...
                            @foreach($tahunAjaran as $ta)
                                <option value="{{ $ta->id }}"
                                    {{ old('tahun_ajaran_id', $tahunAktif?->id) == $ta->id ? 'selected' : '' }}>
            ...
            <div class="form-section">
                ...
                <div class="form-grid-3">
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="durasi_menit" value="{{ old('durasi_menit', 90) }}"
                               placeholder="cth. 90" min="1" max="480"
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="nilai_kkm" value="{{ old('nilai_kkm', 75) }}"
                               placeholder="cth. 75" min="0" max="100"
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="maks_percobaan" value="{{ old('maks_percobaan', 1) }}"
                               min="1" max="10"
                    ...
                    <div class="field col-span-3">
                        ...
                        <textarea name="keterangan" rows="2"
                                  placeholder="Informasi tambahan mengenai ujian ini (opsional)">{{ old('keterangan') }}</textarea>
            ...
            <div class="form-section">
                ...
                <div style="display:flex;flex-direction:column;gap:14px">
                    {{--
                        FIX: Pola hidden + checkbox yang benar.
                        hidden value="0" dikirim jika checkbox tidak dicentang.
                        Jika checkbox dicentang, value="1" yang dikirim (menimpa hidden).
                        Logika old():
                        - acak_soal & acak_pilihan: default OFF → hanya aktif jika old == '1'
                        - tampilkan_nilai & is_active: default ON → aktif jika belum pernah
                          submit (is_null old('_token')) ATAU old == '1'
                    --}}
                    <div class="toggle-row">
                        <label class="toggle-switch">
                            <input type="hidden" name="acak_soal" value="0">
                            <input type="checkbox" name="acak_soal" value="1"
                                   {{ old('acak_soal') == '1' ? 'checked' : '' }}>
                    ...
                    <div class="toggle-row">
                        <label class="toggle-switch">
                            ...
                            <input type="checkbox" name="tampilkan_nilai" value="1"
                                   {{ is_null(old('_token')) ? 'checked' : (old('tampilkan_nilai') == '1' ? 'checked' : '') }}>
                            <span class="toggle-slider"></span>
                        </label>
                    ...
                    <div class="toggle-row">
                        <label class="toggle-switch">
                            ...
                            <input type="checkbox" name="is_active" value="1"
                                   {{ is_null(old('_token')) ? 'checked' : (old('is_active') == '1' ? 'checked' : '') }}>
                            <span class="toggle-slider"></span>
                        </label>
...
<script>
    {{--
        FIX: SweetAlert error menggunakan @json() agar aman dari XSS.
        Tidak menggunakan @foreach langsung di dalam JS string.
    --}}
    @if($errors->any())
    (function () {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Terdapat ' + errors.length + ' Kesalahan',
            html: `<ul style="text-align:left;padding-left:16px;margin:0;display:flex;flex-direction:column;gap:4px">${listHtml}</ul>`,
            confirmButtonColor: '#1f63db'
        });
    })();
    @endif

    // ── Cascade Dropdown: Guru → Mapel & Kelas ───────────────────────────────
    ...
    function fillSelect(selectEl, items, placeholder, labelKey, autoSelectValue) {
        selectEl.innerHTML = `<option value="">${placeholder}</option>`;
        items.forEach(item => {
            const opt = document.createElement('option');
            opt.value = item.id;
            opt.textContent = item[labelKey];
            if (initialLoad && String(item.id) === String(autoSelectValue)) {
                opt.selected = true;
            }
    ...
    async function loadCascade(guruId) {
        if (!guruId) {
        ...
        mapelSelect.disabled = true;
        kelasSelect.disabled = true;
        mapelLoading.style.display = 'block';
        kelasLoading.style.display = 'block';

        try {
            {{-- Route AJAX sesuai web.php: admin.ujian.ajax.mapel-by-guru & admin.ujian.ajax.kelas-by-guru --}}
            const baseMapelUrl = "{{ route('admin.ujian.ajax.mapel-by-guru', ['guru' => '__ID__']) }}";
            const baseKelasUrl = "{{ route('admin.ujian.ajax.kelas-by-guru', ['guru' => '__ID__']) }}";
            ...
            const [mapelRes, kelasRes] = await Promise.all([
                fetch(baseMapelUrl.replace('__ID__', guruId), { headers: ajaxHeaders }),
                fetch(baseKelasUrl.replace('__ID__', guruId), { headers: ajaxHeaders }),
            ]);

            if (!mapelRes.ok || !kelasRes.ok) throw new Error('Gagal mengambil data');

            const mapelData = await mapelRes.json();
            const kelasData = await kelasRes.json();

            const oldMapel = mapelSelect.dataset.old || '';
            const oldKelas = kelasSelect.dataset.old || '';

            fillSelect(mapelSelect, mapelData, '— Pilih Mata Pelajaran —', 'nama_mapel', oldMapel);
            fillSelect(kelasSelect, kelasData, '— Pilih Kelas —', 'nama_kelas', oldKelas);

        } catch (err) {
            console.warn('Cascade dropdown error:', err);
            mapelSelect.innerHTML = '<option value="">— Pilih Mata Pelajaran —</option>';
            kelasSelect.innerHTML = '<option value="">— Pilih Kelas —</option>';
            mapelSelect.disabled = false;
            kelasSelect.disabled = false;
        } finally {
            mapelLoading.style.display = 'none';
