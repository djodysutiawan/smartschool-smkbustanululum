...
.icon-btn.essay:hover{background:var(--purple-bg);color:var(--purple);}
...
<div class="page">
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Kelola Soal — {{ Str::limit($ujian->judul, 40) }}</h1>
            <p class="page-sub">
        ...
        <div class="header-actions">
            ...
            {{-- FIXED: route name sesuai routes_ujian_section.php → admin.ujian.soal.export.pdf --}}
            ...
            <button onclick="document.getElementById('modalImport').classList.add('open')" class="btn btn-export">
                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10" style="transform:scaleY(-1);transform-origin:center"/></svg>
                Import
            </button>
                ...
                <tr data-id="{{ $s->id }}" style="cursor:grab;">
                    ...
                    <td>
                        <div class="action-cell">
                            <a href="{{ route('admin.ujian.soal.edit', [$ujian, $s]) }}"
                               class="icon-btn edit" title="Edit Soal">
                                <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </a>
                            {{-- Tombol koreksi essay hanya muncul untuk soal essay --}}
                            @if($s->jenis_soal === 'essay')
                            <a href="{{ route('admin.ujian.soal.koreksi-essay.index', [$ujian, $s]) }}"
                               class="icon-btn essay" title="Koreksi Jawaban Essay">
                            ...
                            @endif
                            <form action="{{ route('admin.ujian.soal.destroy', [$ujian, $s]) }}"
                                  method="POST" style="display:inline">
                                @csrf @method('DELETE')
                                <button type="submit" class="icon-btn del" title="Hapus Soal"
                                    onclick="return confirm('Hapus soal no.{{ $s->nomor_soal }}?\nSemua jawaban siswa terkait akan ikut terhapus.')">
    ...
    <div class="modal">
        ...
        <form action="{{ route('admin.ujian.soal.import', $ujian) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-footer">
                {{-- FIXED: route name sesuai routes_ujian_section → admin.ujian.soal.import.template --}}
                <a href="{{ route('admin.ujian.soal.import.template', $ujian) }}" class="btn btn-export">
                    Download Template
                </a>
                <button type="button"
                        onclick="document.getElementById('modalImport').classList.remove('open')"
                        class="btn btn-back">Batal</button>
...
if (tbody) {
    new Sortable(tbody, {
        ...
        onEnd: function() {
            ...
            }).catch(() => alert('Gagal menyimpan urutan. Silakan refresh halaman.'));
        }
    });
}
