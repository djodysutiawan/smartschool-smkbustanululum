...
<style>
    ...
    .nama-wrap .email { font-size: 12px; color: var(--text3); margin-top: 1px; }
    ...
    .kelas-pill {
        ...
        border-radius: 5px;
        font-family: 'Plus Jakarta Sans', sans-serif;
        font-size: 12px; font-weight: 700; border: 1px solid var(--brand-100);
    }
...
<div class="page">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Kelola data siswa — tambah, edit, dan ekspor data</p>
        </div>
        <div class="header-actions">
            <a href="{{ route('admin.students.export') }}" class="btn btn-export">
    ...
    <div class="stats-strip">
        <div class="stat-card">
            ...
            <div>
                ...
                <p class="stat-val">{{ $students->total() }}</p>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div>
                ...
                <p class="stat-val">{{ $students->getCollection()->where('status','aktif')->count() }}</p>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon indigo">
                <svg width="18" height="18" fill="none" stroke="#4f46e5" stroke-width="1.8" viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            </div>
            <div>
                <p class="stat-label">Lulus</p>
                <p class="stat-val">{{ $students->getCollection()->where('status','lulus')->count() }}</p>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon yellow">
                <svg width="18" height="18" fill="none" stroke="#a16207" stroke-width="1.8" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </div>
            <div>
                <p class="stat-label">Pindah</p>
                <p class="stat-val">{{ $students->getCollection()->where('status','pindah')->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="table-card">
        <div class="table-topbar">
            <p class="table-info">
                Daftar Siswa
                <span>— menampilkan {{ $students->firstItem() }}–{{ $students->lastItem() }} dari {{ $students->total() }} data</span>
            </p>
        </div>
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="no-col">{{ $students->firstItem() + $index }}</span></td>
                        ...
                        <td>
                            <div class="foto-wrap">
                                ...
                                @else
                                    <span class="foto-initial">{{ strtoupper(substr($s->nama_lengkap, 0, 1)) }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="nama-wrap">
                                <p class="nama">{{ $s->nama_lengkap }}</p>
                                <p class="email">{{ $s->user->email ?? '-' }}</p>
                            </div>
                        </td>

                        <td class="muted" style="font-family:'Plus Jakarta Sans',sans-serif;font-size:12.5px;font-weight:600">
                            {{ $s->nis }}
                        </td>

                        <td>
                            <span class="kelas-pill">{{ $s->class->nama_kelas ?? '-' }}</span>
                        </td>

                        <td class="muted">{{ $s->academicYear->tahun ?? '-' }}</td>
                                ...
                                <form action="{{ route('admin.students.destroy', $s->id) }}"
                                    ...
                                    <button type="button" class="btn btn-sm btn-del"
                                        onclick="confirmDelete(
                                            document.getElementById('deleteForm-{{ $s->id }}'),
        ...
        <div class="pag-wrap">
            ...
            <div class="pag-btns">
                ...
                @else
                    <a href="{{ $students->previousPageUrl() }}" class="pag-btn">
                        <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
                    </a>
                @endif
                ...
                @if($students->hasMorePages())
                    <a href="{{ $students->nextPageUrl() }}" class="pag-btn">
...
{{-- ══════════════════════════════════════════ --}}
{{-- SweetAlert2 — HARUS di dalam x-app-layout --}}
{{-- ══════════════════════════════════════════ --}}
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
    // Flash session success (setelah store/update/destroy)
    @if(session('success'))
    Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
