...
<style>
    ...
    *,*::before,*::after{box-sizing:border-box}
    ...
    .section-title svg{color:var(--brand-600);flex-shrink:0}
    ...
    .field input,
    .field select,
    .field textarea{padding:9px 12px;border:1px solid var(--border);border-radius:var(--radius-sm);font-family:'DM Sans',sans-serif;font-size:13.5px;color:var(--text);background:var(--surface);outline:none;transition:border-color .15s,box-shadow .15s;width:100%}
    .field input:focus,
    .field select:focus,
    .field textarea:focus{border-color:var(--brand-500);box-shadow:0 0 0 3px rgba(53,130,240,.1)}
    ...
    /* ── Siswa preview ── */
    .siswa-preview{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:10px 14px;margin-top:8px;display:none;align-items:center;gap:10px}
    .siswa-preview.show{display:flex}
    ...
    .siswa-meta{font-size:12px;color:var(--text3);margin-top:1px}
    ...
    .btn{display:inline-flex;align-items:center;gap:6px;padding:9px 20px;border-radius:var(--radius-sm);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer;border:none;text-decoration:none;transition:filter .15s,background .15s;line-height:1}
...
<div class="page">
    <div class="page-header">
        <div>
            ...
            <h1 class="page-title">Buat Izin Keluar Siswa</h1>
            <p class="page-sub">Isi data izin keluar siswa dengan lengkap dan benar</p>
        </div>
    </div>

    <form action="{{ route('admin.izin-keluar-siswa.store') }}" method="POST" novalidate>
        @csrf
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-grid cols-1">
                    <div class="field">
                        <label for="siswa_id">Pilih Siswa<span class="req">*</span></label>
                        <select name="siswa_id" id="siswaSelect" class="{{ $errors->has('siswa_id') ? 'is-invalid' : '' }}" required>
                        ...
                        <div class="siswa-preview" id="siswaPreview">
                            ...
                            <div>
                                <p class="siswa-name" id="previewNama">—</p>
                                <p class="siswa-meta" id="previewMeta">—</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="field">
                        <label for="jam_keluar">Jam Keluar<span class="req">*</span></label>
                        <input type="time"
                               name="jam_keluar"
                               id="jamKeluar"
                    ...
                    <div class="field">
                        <label for="jam_kembali">Rencana Jam Kembali</label>
                        <input type="time"
                               name="jam_kembali"
                               id="jamKembali"
                               value="{{ old('jam_kembali') }}"
                               class="{{ $errors->has('jam_kembali') ? 'is-invalid' : '' }}">
                        <p class="field-hint">Opsional — perkiraan waktu kembali ke sekolah</p>
                        @error('jam_kembali')
                            <p class="field-error">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="field span-2">
                        <label for="tujuan">Tujuan / Keperluan<span class="req">*</span></label>
                        <input type="text"
                               name="tujuan"
                               id="tujuan"
                               value="{{ old('tujuan') }}"
                               placeholder="Contoh: Ke dokter gigi, Rumah duka keluarga…"
                               class="{{ $errors->has('tujuan') ? 'is-invalid' : '' }}"
                               maxlength="255"
                               required>
                        @error('tujuan')
                            <p class="field-error">{{ $message }}</p>
            ...
            <div class="form-footer">
                <a href="{{ route('admin.izin-keluar-siswa.index') }}" class="btn btn-secondary">Batal</a>
...
<script>
    ...
    function updateSiswaPreview() {
        const opt = siswaSelect.options[siswaSelect.selectedIndex];
        if (opt && opt.value) {
            // Ambil nama dari teks opsi (sebelum " — ")
            previewNama.textContent = opt.text.split(' — ')[0] ?? opt.text;
            previewMeta.textContent = 'Kelas: ' + (opt.dataset.kelas || '—') + '  ·  NIS: ' + (opt.dataset.nis || '—');
            siswaPreview.classList.add('show');
        } else {
            siswaPreview.classList.remove('show');
        }
    }

    siswaSelect.addEventListener('change', updateSiswaPreview);
    // Tampilkan preview saat halaman dimuat kembali setelah error validasi
    updateSiswaPreview();

    // ── Validasi jam kembali client-side ──
    const jamKeluar  = document.getElementById('jamKeluar');
    const jamKembali = document.getElementById('jamKembali');

    function validateJamKembali() {
        if (jamKembali.value && jamKeluar.value && jamKembali.value <= jamKeluar.value) {
            jamKembali.setCustomValidity('Rencana jam kembali harus setelah jam keluar.');
            jamKembali.classList.add('is-invalid');
        } else {
            jamKembali.setCustomValidity('');
            jamKembali.classList.remove('is-invalid');
        }
    }

    jamKeluar.addEventListener('change', validateJamKembali);
    jamKembali.addEventListener('change', validateJamKembali);
</script>
</x-app-layout>