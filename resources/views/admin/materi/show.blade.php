...
<div class="page">
            ...
            <div style="display:flex;gap:24px;align-items:flex-start;flex-wrap:wrap;margin-bottom:20px">
                ...
                <div style="flex:1;min-width:200px">
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
                        {{-- BUG FIX: Gunakan accessor label_jenis dari model --}}
                        <span class="jenis-pill jenis-{{ $materi->jenis }}">{{ $materi->label_jenis }}</span>
    ...
    <div class="detail-card">
        ...
        <div class="card-body">
            @if($materi->jenis === 'file')
                {{-- BUG FIX: Gunakan accessor file_url dari model, cek null sebelum render --}}
                @if($materi->file_url)
                    <a href="{{ $materi->file_url }}" download class="file-download" target="_blank">
                ...
                @else
                    <p style="color:var(--text3);font-size:13.5px">File belum diunggah.</p>
                @endif

            @elseif($materi->jenis === 'link')
                {{-- BUG FIX: Cek file_url (accessor sudah handle url_eksternal untuk jenis link) --}}
                @if($materi->file_url)
                    <p class="dl-label" style="margin-bottom:8px">URL Link Eksternal</p>
                    <a href="{{ $materi->file_url }}" target="_blank" class="link-external">
                ...
                @else
                    <p style="color:var(--text3);font-size:13.5px">URL eksternal belum diisi.</p>
                @endif

            @elseif($materi->jenis === 'video')
                @if($materi->file_url)
                    <p class="dl-label" style="margin-bottom:8px">URL Video</p>
                    <a href="{{ $materi->file_url }}" target="_blank" class="link-external">
                        <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                        {{ $materi->file_url }}
                    </a>
                    {{--
                        BUG FIX: Regex YouTube diperbaiki agar lebih robust.
                        Mendukung: youtube.com/watch?v=, youtu.be/, youtube.com/embed/,
                        youtube.com/shorts/, dan format dengan parameter tambahan.
                    --}}
                    @php
                        $ytId = null;
                        if (preg_match(
                    ...
                    @endphp
                    @if($ytId)
                    <div style="margin-top:16px;border-radius:var(--radius-sm);overflow:hidden;max-width:560px;aspect-ratio:16/9;">
                        <iframe
                            width="100%"
                            height="100%"
                    ...
                    @endif
                @else
                    <p style="color:var(--text3);font-size:13.5px">URL video belum diisi.</p>
                @endif

            @elseif($materi->jenis === 'teks')
                {{--
                    BUG FIX KRITIS: Field 'konten' tidak ada di model Materi.
                    Model hanya punya field 'deskripsi'. Ganti $materi->konten → $materi->deskripsi.
                --}}
                @if($materi->deskripsi)
                    <div class="deskripsi-box" style="min-height:100px">{{ $materi->deskripsi }}</div>
                @else
                    <p style="color:var(--text3);font-size:13.5px;font-style:italic">(Tidak ada konten teks)</p>
                @endif
    ...
    <div class="detail-card">
        ...
        <div class="meta-row">
            ...
            <span class="meta-item"><strong>Dipublikasikan:</strong> {{ $materi->dipublikasikan_pada->format('d M Y, H:i') }}</span>
            @endif
        </div>
    </div>
</div>
