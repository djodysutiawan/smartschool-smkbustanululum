...
<div class="page">
    ...
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <a href="{{ route('admin.ruang.show', $ruang->id) }}">{{ $ruang->nama_ruang }}</a>
        <span class="sep">›</span>
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Edit Ruang</h1>
            <p class="page-sub">Perbarui data ruang {{ $ruang->nama_ruang }}</p>
        </div>
        <a href="{{ route('admin.ruang.show', $ruang->id) }}" class="btn btn-back">
            <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
            Kembali
        </a>
    </div>

    <form action="{{ route('admin.ruang.update', $ruang->id) }}" method="POST" id="ruangForm">
        @csrf @method('PUT')
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="field">
                        ...
                        <select name="gedung_id" class="{{ $errors->has('gedung_id') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($gedungs as $g)
                                <option value="{{ $g->id }}" {{ old('gedung_id', $ruang->gedung_id) == $g->id ? 'selected' : '' }}>{{ $g->nama_gedung }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <input type="text" name="kode_ruang" value="{{ old('kode_ruang', $ruang->kode_ruang) }}" maxlength="15" class="{{ $errors->has('kode_ruang') ? 'is-invalid' : '' }}">
                        @error('kode_ruang')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Nama Ruang <span class="req">*</span></label>
                        <input type="text" name="nama_ruang" value="{{ old('nama_ruang', $ruang->nama_ruang) }}" maxlength="100" class="{{ $errors->has('nama_ruang') ? 'is-invalid' : '' }}">
                        @error('nama_ruang')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Lantai <span class="req">*</span></label>
                        <input type="number" name="lantai" value="{{ old('lantai', $ruang->lantai) }}" min="1" class="{{ $errors->has('lantai') ? 'is-invalid' : '' }}">
                    ...
                    <div class="field">
                        ...
                        <select name="jenis_ruang" class="{{ $errors->has('jenis_ruang') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($jenisOptions as $j)
                                <option value="{{ $j }}" {{ old('jenis_ruang', $ruang->jenis_ruang) == $j ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$j)) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="kapasitas" value="{{ old('kapasitas', $ruang->kapasitas) }}" min="1" max="500" class="{{ $errors->has('kapasitas') ? 'is-invalid' : '' }}">
                    ...
                    <div class="field">
                        ...
                        <select name="status" class="{{ $errors->has('status') ? 'is-invalid' : '' }}">
                            ...
                            <option value="tidak_tersedia" {{ old('status',$ruang->status)=='tidak_tersedia' ? 'selected':'' }}>Tidak Tersedia</option>
                            <option value="perbaikan"      {{ old('status',$ruang->status)=='perbaikan'      ? 'selected':'' }}>Perbaikan</option>
                        </select>
                        @error('status')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Keterangan</label>
                        <input type="text" name="keterangan" value="{{ old('keterangan', $ruang->keterangan) }}" placeholder="Opsional" maxlength="500">
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                {{--
                    ...
                    FIX BUG: old($name, $ruang->$name) — pada edit, fallback ke nilai model
                    saat tidak ada old input (fresh load). Cast boolean di model sudah
                    memastikan $ruang->ada_proyektor dll. bertipe true/false.
                --}}
                <div class="checkbox-grid">
                    ...
                    @php
                        // Saat ada validation error, ambil dari old(); fresh load ambil dari model
                        $isChecked = old($name, $ruang->$name) ? true : false;
                    @endphp
                    <label class="checkbox-item {{ $isChecked ? 'checked' : '' }}" id="wrap-{{ $name }}">
                        <input type="checkbox" name="{{ $name }}" value="1" {{ $isChecked ? 'checked' : '' }}
                ...
                <div class="field" style="margin-top:14px">
                    ...
                    <input type="text" name="fasilitas_lain" value="{{ old('fasilitas_lain', $ruang->fasilitas_lain) }}" placeholder="cth. Papan tulis digital, Sound system" maxlength="500">
                    <span class="field-hint">Pisahkan dengan koma jika lebih dari satu</span>
                </div>
            </div>

            <div class="form-footer">
                <a href="{{ route('admin.ruang.show', $ruang->id) }}" class="btn btn-cancel">Batal</a>
