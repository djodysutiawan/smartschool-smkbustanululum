...
<style>
    ...
    .teacher-avatar {
        ...
        font-family: 'Plus Jakarta Sans', sans-serif; font-size: 13px;
        font-weight: 800; color: var(--brand-700); overflow: hidden;
    }
    ...
    .teacher-nip { font-size: 12px; color: var(--text3); margin-top: 1px; }
...
<div class="page">
    ...
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <span class="current">Manajemen Guru Piket</span>
    </nav>

    {{-- Page Header --}}
    <div class="page-header">
        <div>
            <h1 class="page-title">Guru Piket</h1>
            <p class="page-sub">Kelola jadwal dan ketersediaan guru piket</p>
        </div>
        <div class="header-actions">
            <button id="btnBulkDelete" class="btn btn-danger btn-sm" style="display:none" onclick="confirmBulkDelete()">
            ...
            <a href="{{ route('admin.piket-teachers.create') }}" class="btn btn-primary">
                <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    {{-- Stats --}}
    @php
        $totalAll = $piketTeachers->total();
        $countHari = $piketTeachers->groupBy('hari')->count(); // approximate, page only
    ...
    <div class="stats-row">
        <div class="stat-card">
            ...
            <div>
                <p class="stat-num">{{ $piketTeachers->total() }}</p>
        ...
        <div class="stat-card">
            ...
            <div>
                <p class="stat-num">{{ $hariList ? count($hariList) : 6 }}</p>
        ...
        <div class="stat-card">
            ...
            <div>
                <p class="stat-num">{{ $piketTeachers->count() }}</p>
                <p class="stat-label">Halaman Ini</p>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div>
                <p class="stat-num">{{ $teachers->count() }}</p>
    ...
    {{-- Filter --}}
    <form method="GET" action="{{ route('admin.piket-teachers.index') }}" class="filter-card">
        <svg width="14" height="14" fill="none" stroke="#94a3b8" stroke-width="2" viewBox="0 0 24 24">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
        </svg>
        <input
            type="text" name="search" placeholder="Cari nama / NIP guru…"
        ...
        @if(request()->hasAny(['search','hari','status']))
            <a href="{{ route('admin.piket-teachers.index') }}" class="btn btn-ghost btn-sm">Reset</a>
        @endif
    </form>

    {{-- Bulk delete form --}}
    <form id="bulkForm" method="POST" action="{{ route('admin.piket-teachers.bulkDelete') }}">
        @csrf @method('DELETE')
        ...
        <div class="table-card">
            <div class="table-header">
                ...
                <p class="table-title">Daftar Jadwal Guru Piket</p>
                @if($piketTeachers->total())
                    <span class="table-badge">{{ $piketTeachers->total() }} jadwal</span>
                @endif
            </div>

            @if($piketTeachers->count())
                <table class="main-table">
                    ...
                    <tbody>
                        @foreach($piketTeachers as $piket)
                            @php
                                $mulai    = \Carbon\Carbon::createFromFormat('H:i:s', $piket->jam_mulai);
                                $selesai  = \Carbon\Carbon::createFromFormat('H:i:s', $piket->jam_selesai);
                                $durasi   = $mulai->diffInMinutes($selesai);
                                $jam      = intdiv($durasi, 60);
                                $mnt      = $durasi % 60;
                                $durasiStr = ($jam > 0 ? $jam.'j ' : '') . ($mnt > 0 ? $mnt.'m' : '');
                            @endphp
                            <tr>
                                <td>
                                    <input type="checkbox" class="cb row-cb" name="ids[]"
                                           value="{{ $piket->id }}" onchange="updateBulk()">
                                </td>
                                <td>
                                    <div class="teacher-cell">
                                        <div class="teacher-avatar">
                                            @if($piket->teacher?->foto)
                                                <img src="{{ asset('storage/'.$piket->teacher->foto) }}" alt="">
                                            @else
                                                {{ strtoupper(substr($piket->teacher?->nama_lengkap ?? '?', 0, 1)) }}
                                            @endif
                                        </div>
                                        <div>
                                            <a href="{{ route('admin.piket-teachers.show', $piket->id) }}" class="teacher-name">
                                                {{ $piket->teacher?->nama_lengkap ?? '—' }}
                                            </a>
                                            <p class="teacher-nip">NIP: {{ $piket->teacher?->nip ?? '—' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="day-badge day-{{ $piket->hari }}">{{ $piket->hari }}</span>
                                </td>
                                <td>
                                    <span class="jam-badge">
                                        ...
                                        {{ \Carbon\Carbon::createFromFormat('H:i:s', $piket->jam_mulai)->format('H:i') }}
                                    </span>
                                </td>
                                <td>
                                    <span class="jam-badge">
                                        ...
                                        {{ \Carbon\Carbon::createFromFormat('H:i:s', $piket->jam_selesai)->format('H:i') }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td class="center">
                                    <div class="actions-cell">
                                        <a href="{{ route('admin.piket-teachers.show', $piket->id) }}"
                                        ...
                                        <a href="{{ route('admin.piket-teachers.edit', $piket->id) }}"
                                        ...
                                        <button type="button"
                                                ...
                                                title="Hapus"
                                                onclick="confirmDelete({{ $piket->id }}, '{{ addslashes($piket->teacher?->nama_lengkap) }}', '{{ $piket->hari }}')">
                                            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    <form id="del-{{ $piket->id }}" method="POST"
                                          action="{{ route('admin.piket-teachers.destroy', $piket->id) }}" style="display:none">
                                        @csrf @method('DELETE')
                                    </form>
                                </td>
                            </tr>
                ...
                {{-- Pagination --}}
                @if($piketTeachers->hasPages())
                    <div class="pagination-wrap">
                        <p class="pagination-info">
                            Menampilkan {{ $piketTeachers->firstItem() }}–{{ $piketTeachers->lastItem() }}
                            dari {{ $piketTeachers->total() }} jadwal
                        </p>
                        <div class="pagination-links">
                            @if($piketTeachers->onFirstPage())
                                <span class="page-link disabled">‹</span>
                            @else
                                <a class="page-link" href="{{ $piketTeachers->previousPageUrl() }}">‹</a>
                            @endif

                            @foreach($piketTeachers->getUrlRange(1, $piketTeachers->lastPage()) as $page => $url)
                                <a class="page-link {{ $page == $piketTeachers->currentPage() ? 'active' : '' }}"
                                   href="{{ $url }}">{{ $page }}</a>
                            @endforeach

                            @if($piketTeachers->hasMorePages())
                                <a class="page-link" href="{{ $piketTeachers->nextPageUrl() }}">›</a>
                            @else
                                <span class="page-link disabled">›</span>
                            @endif
                        </div>
            ...
            @else
                <div class="empty-state">
                    ...
                    <p class="empty-sub">
                        ...
                        @else
                            Klik "Tambah Jadwal" untuk menambahkan jadwal guru piket
                        @endif
                    </p>
                    @if(!request()->hasAny(['search','hari','status']))
                        <a href="{{ route('admin.piket-teachers.create') }}" class="btn btn-primary btn-sm" style="margin-top:8px">
                            Tambah Sekarang
                        </a>
                    @endif
                </div>
...
<script>
    ...
    function confirmDelete(id, nama, hari) {
        Swal.fire({
            title: 'Hapus Jadwal?',
            html: `Jadwal piket <b>${nama}</b> hari <b>${hari}</b> akan dihapus permanen.`,
            icon: 'warning',
