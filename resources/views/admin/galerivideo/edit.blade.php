...
<style>
    ...
    .card-header { padding:14px 20px;border-bottom:1px solid var(--border);font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:700;color:var(--text);display:flex;align-items:center;justify-content:space-between;gap:8px; }
    ...
    /* Current video badge */
    .current-badge { display:inline-flex;align-items:center;gap:6px;padding:5px 12px;background:var(--brand-50);border:1px solid var(--brand-100);border-radius:99px;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:700;color:var(--brand-700);margin-bottom:14px; }

    /* Upload area */
    .upload-area { border:2px dashed var(--border2);border-radius:var(--radius);padding:20px;text-align:center;cursor:pointer;transition:all .15s;position:relative; }
...
<form action="{{ route('admin.galeri.video.destroy', $galeriVideo) }}" method="POST" id="deleteForm">
    @csrf @method('DELETE')
</form>

<div class="page">
    <div class="breadcrumb">
        <a href="{{ route('admin.galeri.video.index') }}">Galeri Video</a>
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">{{ $galeriVideo->judul }}</p>
        </div>
    ...
    <form action="{{ route('admin.galeri.video.update', $galeriVideo) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="layout">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror"
                                value="{{ old('judul', $galeriVideo->judul) }}" placeholder="Judul video...">
                            @error('judul')<p class="invalid-feedback">{{ $message }}</p>@enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" rows="3"
                                placeholder="Deskripsi singkat video...">{{ old('deskripsi', $galeriVideo->deskripsi) }}</textarea>
                        </div>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                                <div>
                                    ...
                                    <input type="text" name="durasi" class="form-control"
                                        value="{{ old('durasi', $galeriVideo->durasi) }}" placeholder="cth. 3:45">
                                </div>
                                <div>
                                    <label class="form-label">Tanggal Video</label>
                                    <input type="date" name="tanggal_video" class="form-control"
                                        value="{{ old('tanggal_video', $galeriVideo->tanggal_video?->format('Y-m-d')) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="sumber-tabs">
                            @php $currentTipe = old('tipe_sumber', $galeriVideo->tipe_sumber); @endphp
                            <label class="sumber-opt">
                                <input type="radio" name="tipe_sumber" value="youtube"
                                    {{ $currentTipe === 'youtube' ? 'checked' : '' }}
                                    onchange="switchSumber('youtube')">
                            ...
                            <label class="sumber-opt">
                                <input type="radio" name="tipe_sumber" value="vimeo"
                                    {{ $currentTipe === 'vimeo' ? 'checked' : '' }}
                                    onchange="switchSumber('vimeo')">
                                <span class="sumber-label">
                            ...
                            <label class="sumber-opt">
                                <input type="radio" name="tipe_sumber" value="upload"
                                    {{ $currentTipe === 'upload' ? 'checked' : '' }}
                                    onchange="switchSumber('upload')">
                                <span class="sumber-label">
                        ...
                        {{-- Panel YouTube / Vimeo --}}
                        <div id="panel-embed">
                            {{-- Preview embed saat ini --}}
                            @if($galeriVideo->video_embed_url)
                            <div class="video-preview-wrap" id="embedPreviewWrap">
                                <iframe id="embedPreviewFrame"
                                    src="{{ old('video_embed_url', $galeriVideo->video_embed_url) }}"
                            ...
                            <input type="hidden" name="video_embed_id"  id="videoEmbedId"
                                value="{{ old('video_embed_id', $galeriVideo->video_embed_id) }}">
                            <input type="hidden" name="video_embed_url" id="videoEmbedUrl"
                                value="{{ old('video_embed_url', $galeriVideo->video_embed_url) }}">

                            <div class="form-group" style="margin-bottom:0">
                                <label class="form-label">Sumber / Channel</label>
                                <input type="text" name="sumber" class="form-control"
                                    value="{{ old('sumber', $galeriVideo->sumber) }}" placeholder="Nama channel atau sumber">
                            </div>
                        </div>

                        {{-- Panel Upload --}}
                        <div id="panel-upload" style="display:none">
                            @if($galeriVideo->video_path)
                            <div class="current-badge">
                                ...
                                File video tersimpan — ganti jika perlu
                            </div>
                            @endif
                            <div class="form-group">
                                <label class="form-label">Ganti File Video (opsional)</label>
                                <div class="upload-area">
                                    <input type="file" name="video" accept="video/mp4,video/webm,video/quicktime" onchange="showFileName(this)">
                                    <p class="upload-area-text"><strong>Klik untuk upload</strong> atau seret file video</p>
                                    <p class="form-hint" id="fileNameDisplay">MP4, WEBM, MOV — maks 200 MB</p>
                                </div>
                            </div>
                            <div class="form-group" style="margin-bottom:0">
                                <label class="form-label">Sumber</label>
                                <input type="text" name="sumber" class="form-control"
                                    value="{{ old('sumber', $galeriVideo->sumber) }}" placeholder="Nama pembuat / sumber video">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="card">
                    ...
                    <div class="card-body" style="display:flex;flex-direction:column;gap:8px">
                        ...
                        <button type="button" class="btn btn-danger" style="width:100%;justify-content:center" onclick="confirmDelete()">
                        ...
                        <p style="font-size:11.5px;color:var(--text3);text-align:center">
                            Diunggah {{ $galeriVideo->created_at->diffForHumans() }}
                        </p>
                    </div>
                </div>
                        ...
                        <div class="form-group" style="margin-bottom:0">
                            ...
                            <select name="galeri_kategori_id" class="form-control @error('galeri_kategori_id') is-invalid @enderror">
                                ...
                                @foreach($kategori as $k)
                                    <option value="{{ $k->id }}"
                                        {{ old('galeri_kategori_id', $galeriVideo->galeri_kategori_id) == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="thumb-preview-wrap" id="thumbWrap">
                            ...
                            @php
                                $thumbSrc = $galeriVideo->thumbnail_path
                                    ? Storage::url($galeriVideo->thumbnail_path)
                                    : ($galeriVideo->thumbnail_url ?? null);
                            @endphp
                            @if($thumbSrc)
                                <div id="thumbPlaceholder" style="display:none">
                                    <p class="form-hint"><strong style="color:var(--brand-600)">Klik untuk ganti</strong></p>
                                </div>
                                <img id="thumbPreviewImg" src="{{ $thumbSrc }}" alt="Thumbnail">
                            @else
                                <div id="thumbPlaceholder">
                                    ...
                                    <p class="form-hint" style="margin-top:6px"><strong style="color:var(--brand-600)">Klik untuk ganti</strong> thumbnail</p>
                                </div>
                        ...
                        <div class="form-group" style="margin-top:12px;margin-bottom:0">
                            ...
                            <input type="url" name="thumbnail_url" id="thumbnailUrlInput" class="form-control"
                                value="{{ old('thumbnail_url', $galeriVideo->thumbnail_url) }}" placeholder="https://...">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="toggle-wrap">
                            ...
                            <label class="toggle">
                                ...
                                <input type="checkbox" name="is_published" value="1"
                                    {{ old('is_published', $galeriVideo->is_published) ? 'checked' : '' }}>
                        ...
                        <div class="toggle-wrap">
                            ...
                            <label class="toggle">
                                ...
                                <input type="checkbox" name="is_featured" value="1"
                                    {{ old('is_featured', $galeriVideo->is_featured) ? 'checked' : '' }}>
                                <span class="toggle-slider"></span>
                            </label>
                        </div>
                        <div class="toggle-wrap">
                            <div><p class="toggle-label">Urutan</p></div>
                            <input type="number" name="urutan" min="0" class="form-control"
                                style="width:70px;text-align:center"
                                value="{{ old('urutan', $galeriVideo->urutan) }}">
                        </div>
                    </div>
                </div>
            </div>
...
// Init panel sesuai tipe sumber saat ini
switchSumber('{{ old('tipe_sumber', $galeriVideo->tipe_sumber) }}');
...
async function parseVideoUrl() {
    ...
    try {
        ...
        if (data.embed_url) {
            const wrap  = document.getElementById('embedPreviewWrap');
        ...
        btn.disabled = false;
        btn.innerHTML = '<svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg> Parse';
    }
}
...
function confirmDelete() {
    Swal.fire({
        title: 'Hapus Video?',
        text: '"{{ addslashes($galeriVideo->judul) }}" akan dihapus permanen.',
        icon: 'warning',
