...
/* Meta info box */
.meta-box { background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:14px; }
.meta-row { display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border); }
.meta-row:last-child { border-bottom:none; }
.meta-key { font-size:12px;color:var(--text3); }
.meta-val { font-family:'Plus Jakarta Sans',sans-serif;font-size:12.5px;font-weight:700;color:var(--text); }

.badge-count { display:inline-flex;align-items:center;justify-content:center;min-width:24px;height:20px;padding:0 7px;background:var(--brand-50);color:var(--brand-700);border-radius:99px;font-family:'Plus Jakarta Sans',sans-serif;font-size:11.5px;font-weight:800; }
...
<div class="page">
    <div class="breadcrumb">
        ...
        <span>Edit — {{ $beritaKategori->nama }}</span>
    </div>

    <div class="page-header">
        <div>
            <h1 class="page-title">Edit Kategori Berita</h1>
            <p class="page-sub">Perbarui data kategori "{{ $beritaKategori->nama }}"</p>
        </div>
    ...
    <form action="{{ route('admin.berita-kategori.update', $beritaKategori->id) }}" method="POST">
        @csrf @method('PUT')
        <div class="form-layout">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nama"
                                class="form-control"
                                value="{{ old('nama', $beritaKategori->nama) }}"
                                required
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="slugPreview"
                                ...
                                readonly
                                value="{{ old('nama') ? Str::slug(old('nama')) : $beritaKategori->slug }}">
                            <span class="form-hint">Dibuat otomatis dari nama kategori.</span>
                        </div>

                        <div class="form-group">
                            <label class="form-label">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" rows="3"
                                placeholder="Deskripsi singkat (opsional)...">{{ old('deskripsi', $beritaKategori->deskripsi) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label class="form-label">Warna Label</label>
                            @php $currentColor = old('warna', $beritaKategori->warna ?? '#3582f0'); @endphp
                            <div class="color-row">
                                ...
                                <div class="color-text" id="colorDisplay"
                                    ...
                                    <span id="colorHexLabel">{{ $currentColor }}</span>
                                </div>
                            </div>
                            <input type="hidden" name="warna" id="colorHidden" value="{{ $currentColor }}">
                            <div class="swatches">
                                @foreach(['#3582f0','#15803d','#dc2626','#a16207','#7c3aed','#0891b2','#c2410c','#be185d','#475569','#0f172a'] as $c)
                                    <span class="swatch {{ $currentColor === $c ? 'picked' : '' }}"
                                        style="background:{{ $c }};"
                                        onclick="setColor('{{ $c }}')"
                                        title="{{ $c }}"></span>
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="urutan"
                                class="form-control"
                                value="{{ old('urutan', $beritaKategori->urutan) }}"
                                min="0"
                                placeholder="Kosongkan → pertahankan nilai sekarang">
            ...
            <div style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <div class="form-card-icon">
                            <svg width="15" height="15" fill="none" stroke="#1f63db" stroke-width="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                        </div>
                        <p class="form-card-title">Info Kategori</p>
                    </div>
                    <div class="form-card-body">
                        <div class="meta-box">
                            <div class="meta-row">
                                <span class="meta-key">ID</span>
                                <span class="meta-val">#{{ $beritaKategori->id }}</span>
                            </div>
                            <div class="meta-row">
                                <span class="meta-key">Jumlah Berita</span>
                                <span class="badge-count">{{ $beritaKategori->berita()->count() }}</span>
                            </div>
                            <div class="meta-row">
                                <span class="meta-key">Dibuat</span>
                                <span class="meta-val">{{ $beritaKategori->created_at->format('d M Y') }}</span>
                            </div>
                            <div class="meta-row">
                                <span class="meta-key">Diperbarui</span>
                                <span class="meta-val">{{ $beritaKategori->updated_at->format('d M Y') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="toggle-row">
                            ...
                            <label class="switch">
                                ...
                                <input type="checkbox" name="is_published" value="1"
                                    {{ old('is_published', $beritaKategori->is_published) ? 'checked' : '' }}>
