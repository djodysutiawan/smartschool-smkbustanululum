...
<style>
    ...
    .jurusan-item-badge { font-size: 11px; font-weight: 600; background: var(--brand-100); color: var(--brand-700); padding: 2px 8px; border-radius: 99px; font-family: 'Plus Jakarta Sans', sans-serif; white-space: nowrap; flex-shrink: 0; }
    ...
    <form action="{{ route('admin.mata-pelajaran.update', $mataPelajaran->id) }}" method="POST" id="mapelForm">
        ...
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="field" style="justify-content:flex-end;padding-bottom:4px">
                        ...
                        {{-- FIX #19: Cast boolean ke '1'/'0' string sebelum dibandingkan
                             agar old('is_active', false) tidak dievaluasi sebagai truthy --}}
                        @php
                            $isActiveVal = old('is_active', $mataPelajaran->is_active ? '1' : '0');
                            $isActiveBool = $isActiveVal === '1' || $isActiveVal === 1 || $isActiveVal === true;
                        @endphp
                        <div class="toggle-row" style="margin-top:8px">
                            <label class="toggle-switch">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" name="is_active" value="1" id="isActiveToggle"
                                    {{ $isActiveBool ? 'checked' : '' }}>
            ...
            <div class="form-section">
                ...
                @php $currentScope = old('scope', $mataPelajaran->scope); @endphp
                ...
                <div id="jurusanSection" class="{{ $currentScope === 'jurusan' ? 'visible' : '' }}">
                    <div style="margin-bottom:10px">
                        ...
                        <p class="field-hint">
                            Jurusan yang sudah terhubung ditandai badge
                            <span style="background:var(--brand-100);color:var(--brand-700);padding:1px 6px;border-radius:99px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;font-size:11px">Terhubung</span>.
                        </p>
                    </div>
                    ...
                    <div class="jurusan-grid">
                        ...
                        @php
                            $oldIds     = old('jurusan_ids', null);
                            $existPivot = $pivotExisting->get($jurusan->id);

                            if ($oldIds !== null) {
                                $isChecked    = in_array((string)$jurusan->id, array_map('strval', (array)$oldIds));
                                $pivotJam     = old("jam_jurusan.{$jurusan->id}");
                                $pivotTingkat = old("tingkat_jurusan.{$jurusan->id}");
                            } else {
                                $isChecked    = $existPivot !== null;
                                $pivotJam     = $existPivot['jam_per_minggu'] ?? null;
                                $pivotTingkat = $existPivot['tingkat'] ?? null;
                            }

                            $hasExistingPivot = $existPivot !== null;
                        ...
                        <div class="jurusan-item {{ $isChecked ? 'selected' : '' }}"
                            ...
                            <div class="jurusan-item-header">
                                <label class="jurusan-check-label" for="jCheck-{{ $jurusan->id }}">
                                    ...
                                    @if($hasExistingPivot)
                                        <span class="jurusan-item-badge">Terhubung</span>
                                    @endif
                                ...
                                <div class="jurusan-sub-grid">
                                    ...
                                    <div class="field">
                                        ...
                                        <select name="tingkat_jurusan[{{ $jurusan->id }}]">
                                            ...
                                            <option value="10" {{ (string)$pivotTingkat === '10' ? 'selected' : '' }}>Kelas 10</option>
                                            <option value="11" {{ (string)$pivotTingkat === '11' ? 'selected' : '' }}>Kelas 11</option>
                                            <option value="12" {{ (string)$pivotTingkat === '12' ? 'selected' : '' }}>Kelas 12</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="field">
                        ...
                        <input type="number" name="jam_per_minggu"
                    ...
                    <div class="field">
                        ...
                        <input type="number" name="durasi_per_sesi"
                            value="{{ old('durasi_per_sesi', $mataPelajaran->durasi_per_sesi) }}"
                            min="30" max="180"
                    ...
                    <div class="field col-span-2">
                        ...
                        {{-- FIX #20: Cast perlu_lab ke boolean sebelum cek old() --}}
                        @php
                            $perluLabVal = old('perlu_lab', $mataPelajaran->perlu_lab ? '1' : '0');
                            $perluLabBool = $perluLabVal === '1' || $perluLabVal === 1 || $perluLabVal === true;
                        @endphp
                        <label class="checkbox-item">
                            <input type="hidden" name="perlu_lab" value="0">
                            <input type="checkbox" name="perlu_lab" value="1"
                                {{ $perluLabBool ? 'checked' : '' }}>
...
<script>
    ...
    {{-- FIX #21: @json() encode untuk cegah XSS di SweetAlert html --}}
    @if($errors->any())
    Swal.fire({
        icon: 'error',
