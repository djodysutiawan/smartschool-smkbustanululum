...
<style>
    ...
    .form-input:focus, .form-select:focus { border-color:var(--brand); background:var(--surface); }
    ...
    /* Current file */
    .current-file { display:flex; align-items:center; gap:10px; background:var(--surface2); border:1px solid var(--border); border-radius:var(--radius-sm); padding:12px 14px; margin-bottom:12px; }
    .current-file-icon { width:32px; height:32px; border-radius:8px; background:var(--purple-50); border:1px solid var(--purple-100); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
    .current-file-name { font-family:'Plus Jakarta Sans',sans-serif; font-size:13px; font-weight:700; color:var(--text); }
    .current-file-sub  { font-size:12px; color:var(--text3); font-family:'DM Sans',sans-serif; margin-top:1px; }

    /* File upload */
    .file-upload-area { border:2px dashed var(--border2); border-radius:var(--radius-sm); padding:20px; text-align:center; cursor:pointer; transition:border-color .15s, background .15s; background:var(--surface2); }
    ...
    .file-upload-sub  { font-size:12px; color:var(--text3); font-family:'DM Sans',sans-serif; margin-top:3px; }
    .file-selected    { font-family:'Plus Jakarta Sans',sans-serif; font-size:13px; font-weight:700; color:var(--brand); margin-top:6px; }
...
<div class="page">
    ...
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <a href="{{ route('admin.assignment-submissions.show', $submission->id) }}">Detail</a>
        <span class="sep">›</span>
    ...
    <div class="page-header">
        ...
        <a href="{{ route('admin.assignment-submissions.show', $submission->id) }}" class="btn btn-ghost" style="padding:8px 14px;font-size:12px;">
    ...
    <form method="POST" action="{{ route('admin.assignment-submissions.update', $submission->id) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="card">
            ...
            <div class="card-body">
                @if($submission->file_path)
                    <p style="font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">File Saat Ini</p>
                    <div class="current-file">
                        <div class="current-file-icon">
                            <svg width="16" height="16" fill="none" stroke="#7c3aed" stroke-width="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                        </div>
                        <div>
                            <p class="current-file-name">{{ basename($submission->file_path) }}</p>
                            <p class="current-file-sub">{{ strtoupper(pathinfo($submission->file_path, PATHINFO_EXTENSION)) }} · File saat ini</p>
                        </div>
                    </div>
                @endif
                <div class="form-group" style="margin-bottom:0">
                    <label class="form-label">{{ $submission->file_path ? 'Ganti File (opsional)' : 'Upload File' }}</label>
                    <label class="file-upload-area" for="fileInput">
                        ...
                        <p class="file-upload-text">Klik untuk upload file baru</p>
            ...
            <div class="card-body">
                <div class="form-grid-2">
                    <div class="form-group">
                        ...
                        <input type="number" name="nilai" min="0" max="100" class="form-input {{ $errors->has('nilai') ? 'is-invalid' : '' }}"
                               value="{{ old('nilai', $submission->nilai) }}" placeholder="Kosongkan jika belum dinilai">
                ...
                <div class="form-group" style="margin-bottom:0">
                    ...
                    <input type="datetime-local" name="submitted_at" class="form-input {{ $errors->has('submitted_at') ? 'is-invalid' : '' }}"
                           value="{{ old('submitted_at', $submission->submitted_at ? \Carbon\Carbon::parse($submission->submitted_at)->format('Y-m-d\TH:i') : '') }}">
        ...
        <div class="form-actions">
            <a href="{{ route('admin.assignment-submissions.show', $submission->id) }}" class="btn btn-ghost">Batal</a>
