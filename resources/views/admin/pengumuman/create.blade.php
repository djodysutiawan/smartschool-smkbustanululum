...
<div class="page">
    <nav class="breadcrumb">
        ...
        <span class="sep">›</span>
        <span class="current">Buat Pengumuman</span>
    </nav>

    <div class="page-header">
        <div>
            <h1 class="page-title">Buat Pengumuman Baru</h1>
            <p class="page-sub">Tulis pengumuman dan pilih target penerima</p>
        </div>
    ...
    <form action="{{ route('admin.pengumuman.store') }}" method="POST" enctype="multipart/form-data" id="formPengumuman">
        @csrf
        <div class="form-card">
                ...
                <div class="form-grid">
                    <div class="field col-span-2">
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}"
                            placeholder="cth. Libur Hari Raya Idul Fitri 1446 H"
                    ...
                    <div class="field col-span-2">
                        ...
                        <textarea name="isi" class="isi-textarea {{ $errors->has('isi') ? 'is-invalid' : '' }}"
                            placeholder="Tuliskan isi pengumuman selengkap mungkin...">{{ old('isi') }}</textarea>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="field">
                        ...
                        <select name="target_role" class="{{ $errors->has('target_role') ? 'is-invalid' : '' }}">
                            ...
                            @foreach(['semua' => 'Semua Pengguna','guru' => 'Guru','siswa' => 'Siswa','orang_tua' => 'Orang Tua','guru_piket' => 'Guru Piket'] as $val => $label)
                                <option value="{{ $val }}" {{ old('target_role') == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <span class="field-hint">Opsional — pengumuman otomatis tidak aktif setelah tanggal ini.</span>
                        @error('kadaluarsa_pada')<span class="field-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="field">
                        <label>Langsung Publikasikan</label>
                        <div class="toggle-row">
                            <label class="toggle-switch">
                                <input type="checkbox" name="publikasikan_sekarang" value="1" id="pubToggle"
                                    {{ old('publikasikan_sekarang') ? 'checked' : '' }}>
                                <span class="toggle-slider"></span>
                            </label>
                            <span class="toggle-label" id="toggleLabel">Simpan sebagai Draft</span>
                        </div>
                        <span class="field-hint" style="margin-top:4px">Jika aktif, pengumuman langsung dipublikasikan setelah disimpan.</span>
                    </div>
                    <div class="field">
                        ...
                        <div class="toggle-row">
                            <label class="toggle-switch">
                                <input type="checkbox" name="dipinned" value="1" id="pinnedToggle"
                                    {{ old('dipinned') ? 'checked' : '' }}>
                                <span class="toggle-slider" style=""></span>
                            </label>
                            <span class="toggle-label">Tidak Dipinned</span>
                        </div>
                        <span class="field-hint" style="margin-top:4px">Pengumuman yang di-pin akan tampil paling atas.</span>
                    </div>
                </div>
            </div>
                ...
                <div class="field" style="max-width:520px">
                    <div class="file-upload-wrap">
                        ...
                        <div class="file-upload-box">
                            ...
                            <div>
                                <p class="file-upload-text">Klik untuk pilih file lampiran</p>
...
<script>
    ...
    document.getElementById('pubToggle').addEventListener('change', function() {
        document.getElementById('toggleLabel').textContent = this.checked ? 'Langsung Dipublikasikan' : 'Simpan sebagai Draft';
    });

    document.getElementById('pinnedToggle').addEventListener('change', function() {
        this.closest('.toggle-row').querySelector('.toggle-label').textContent = this.checked ? 'Dipinned' : 'Tidak Dipinned';
        this.previousElementSibling.style.background = this.checked ? '#a16207' : '';
    });
