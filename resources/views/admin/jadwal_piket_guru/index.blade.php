...
<style>
    ...
    .today-chip .chip-jam{opacity:.7;font-weight:400;margin-left:4px}
    .today-empty{opacity:.7;font-size:13px}
    /* Filter */
    ...
    .field input[type="file"]{height:auto;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-sm);font-family:'DM Sans',sans-serif;font-size:13px;color:var(--text);background:var(--surface2);width:100%;outline:none}
    .field input[type="file"]:focus{border-color:var(--brand-500);background:#fff}
    .btn-cancel-modal{background:var(--surface);color:var(--text2);border:1px solid var(--border)}
    /* Catatan cell truncate */
    .catatan-cell{max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12.5px}
...
<div class="page">
    ...
    {{-- Stats — menggunakan nilai dari DB langsung (bukan paginated collection) --}}
    <div class="stats-strip">
        ...
        <div class="stat-card">
            ...
            <div>
                ...
                <p class="stat-val">{{ $piketHariIni->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="today-card">
        <p class="today-title">🛡️ Guru Piket Hari Ini — {{ ucfirst(\App\Models\JadwalPiketGuru::getNamaHari(now())) }}</p>
        @if($piketHariIni->isNotEmpty())
            <div class="today-list">
                ...
                <div class="today-chip">
                    ...
                    @if($p->sudah_absen)
                        <span style="margin-left:4px;font-size:11px;background:rgba(255,255,255,.25);padding:1px 6px;border-radius:4px">✓ {{ ucfirst($p->status_absensi ?? '') }}</span>
                    @endif
                </div>
                @endforeach
            </div>
        @else
            <p class="today-empty">Tidak ada guru yang dijadwalkan piket hari ini.</p>
        @endif
    </div>
    ...
    <div class="table-card">
        <div class="table-topbar">
            ...
            <div class="table-actions">
                {{-- Bangun query string dengan benar agar tidak dobel ? --}}
                @php
                    $qs = request()->query() ? '?' . http_build_query(request()->query()) : '';
                @endphp
                <a href="{{ route('admin.jadwal-piket-guru.export-pdf') }}{{ $qs }}" class="btn btn-sm btn-pdf" target="_blank">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                    Export PDF
                </a>
                <a href="{{ route('admin.jadwal-piket-guru.export-excel') }}{{ $qs }}" class="btn btn-sm btn-export">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="hari-pill hari-{{ $j->hari }}">{{ ucfirst($j->hari) }}</span>
                        </td>
                        ...
                        <td>
                            <span class="catatan-cell muted" title="{{ $j->catatan }}">
                                {{ $j->catatan ?: '-' }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td class="center">
                            <div class="action-group">
                                ...
                                <form action="{{ route('admin.jadwal-piket-guru.destroy', $j->id) }}" method="POST" id="delPG-{{ $j->id }}" style="display:inline">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn btn-sm btn-del"
                                        onclick="confirmDelete('delPG-{{ $j->id }}', '{{ addslashes($j->guru->nama_lengkap ?? 'jadwal ini') }}')">
                                        Hapus
                                    </button>
                                </form>
                            </div>
...
{{-- Modal Import --}}
<div class="modal-overlay" id="modalImport" role="dialog" aria-modal="true" aria-labelledby="modalImportTitle">
    <div class="modal-box">
        <p class="modal-title" id="modalImportTitle">Import Jadwal Piket</p>
        <p class="modal-sub">
            Upload file Excel (.xlsx/.xls) sesuai format template.
            <a href="{{ route('admin.jadwal-piket-guru.import-template') }}" style="color:var(--brand-600);font-weight:600">Download template</a>
        </p>
        <form action="{{ route('admin.jadwal-piket-guru.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="field">
                <label for="importFile">File Excel <span style="color:var(--brand-600)">*</span></label>
                <input type="file" id="importFile" name="file" accept=".xlsx,.xls" required>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-cancel-modal" onclick="closeImportModal()">Batal</button>
...
<script>
    ...
    function confirmDelete(formId, nama) {
        Swal.fire({
            ...
            text: `Jadwal piket "${nama}" akan dihapus secara permanen dan tidak dapat dikembalikan.`,
            icon: 'warning',
