...
<div class="page">
    ...
    <form action="{{ route('admin.jadwal-piket-guru.update', $jadwalPiketGuru->id) }}" method="POST" id="formEditPiket" novalidate>
        @csrf
        @method('PUT')
        <div class="form-card">
                ...
                <div class="form-grid">
                    ...
                    <div class="field">
                        ...
                        <select id="guru_id" name="guru_id" class="{{ $errors->has('guru_id') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($guruPiket as $g)
                                <option value="{{ $g->id }}"
                                    {{ old('guru_id', $jadwalPiketGuru->guru_id) == $g->id ? 'selected' : '' }}>
                                    {{ $g->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <select id="tahun_ajaran_id" name="tahun_ajaran_id" class="{{ $errors->has('tahun_ajaran_id') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($tahunAjaran as $ta)
                                <option value="{{ $ta->id }}"
                                    {{ old('tahun_ajaran_id', $jadwalPiketGuru->tahun_ajaran_id) == $ta->id ? 'selected' : '' }}>
                                    {{ $ta->tahun }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <select id="hari" name="hari" class="{{ $errors->has('hari') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($hariList as $h)
                                <option value="{{ $h }}"
                                    {{ old('hari', $jadwalPiketGuru->hari) == $h ? 'selected' : '' }}>
                                    {{ ucfirst($h) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="field">
                        ...
                        <input type="time" id="jam_mulai" name="jam_mulai"
                            value="{{ old('jam_mulai', \Carbon\Carbon::parse($jadwalPiketGuru->jam_mulai)->format('H:i')) }}"
                    ...
                    <div class="field">
                        ...
                        <input type="time" id="jam_selesai" name="jam_selesai"
                            value="{{ old('jam_selesai', \Carbon\Carbon::parse($jadwalPiketGuru->jam_selesai)->format('H:i')) }}"
                    ...
                    <div class="field" style="justify-content:flex-end;padding-bottom:4px">
                        ...
                        <div class="toggle-row" style="margin-top:8px">
                            ...
                            <span class="toggle-label" id="toggleLabel">
                                {{ old('is_active', $jadwalPiketGuru->is_active) ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="field col-span-2">
                        ...
                        <textarea id="catatan" name="catatan"
                            ...
                            class="{{ $errors->has('catatan') ? 'is-invalid' : '' }}">{{ old('catatan', $jadwalPiketGuru->catatan) }}</textarea>
...
<script>
    ...
    // ── Submit — disable tombol setelah klik ──────────────────────────────────
