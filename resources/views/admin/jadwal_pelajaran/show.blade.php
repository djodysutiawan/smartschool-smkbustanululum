...
<div class="page">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            ...
            <form action="{{ route('admin.jadwal-pelajaran.toggle-status', $jadwalPelajaran->id) }}" method="POST" style="display:inline">
                @csrf @method('PATCH')
                <button type="button" class="btn btn-toggle"
                    onclick="confirmToggle(this.closest('form'), '{{ $jadwalPelajaran->is_active ? 'nonaktifkan' : 'aktifkan' }}')">
    ...
    <div class="detail-grid">
        <div class="detail-card">
            <div class="detail-card-header">
                ...
                <p class="detail-card-title">Informasi Jadwal</p>
            </div>
            <div class="detail-body">
                <div class="detail-row">
                    <span class="detail-label">Hari</span>
                    <span class="detail-val"><span class="hari-pill hari-{{ $jadwalPelajaran->hari }}">{{ ucfirst($jadwalPelajaran->hari) }}</span></span>
                </div>
                <div class="detail-row">
                    ...
                    <div>
                        ...
                        <span style="font-size:12px;color:var(--text3);margin-left:8px">({{ $jadwalPelajaran->durasi_menit }} menit)</span>
                    </div>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Mata Pelajaran</span>
                    <span class="detail-val" style="font-weight:700">{{ $jadwalPelajaran->mataPelajaran->nama_mapel ?? '-' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Kelas</span>
                    <span class="detail-val">{{ $jadwalPelajaran->kelas->nama_kelas ?? '-' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Ruang</span>
                    <span class="detail-val">{{ $jadwalPelajaran->ruang->nama_ruang ?? '-' }}</span>
                </div>
                @if($jadwalPelajaran->pertemuan_ke)
                <div class="detail-row">
                    <span class="detail-label">Pertemuan Ke</span>
                    <span class="detail-val">{{ $jadwalPelajaran->pertemuan_ke }}</span>
                </div>
                @endif
                <div class="detail-row">
        ...
        <div class="detail-card">
            <div class="detail-card-header">
                ...
                <p class="detail-card-title">Guru & Tahun Ajaran</p>
            </div>
            <div class="detail-body">
                <div class="detail-row">
                    <span class="detail-label">Nama Guru</span>
                    <span class="detail-val" style="font-weight:700">{{ $jadwalPelajaran->guru->nama_lengkap ?? '-' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">NIP</span>
                    <span class="detail-val">{{ $jadwalPelajaran->guru->nip ?? '-' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Tahun Ajaran</span>
                    <span class="detail-val">{{ $jadwalPelajaran->tahunAjaran->tahun ?? '-' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Sumber Jadwal</span>
                    <span class="detail-val" style="text-transform:capitalize">{{ $jadwalPelajaran->sumber_jadwal ?? 'Manual' }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Dibuat</span>
                    <span class="detail-val">{{ $jadwalPelajaran->created_at->format('d M Y, H:i') }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Terakhir Diperbarui</span>
                    <span class="detail-val">{{ $jadwalPelajaran->updated_at->format('d M Y, H:i') }}</span>
                </div>
            </div>
        </div>
    </div>
