...
.meta-row{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--surface3);font-size:12.5px;}
...
<div class="page">
    <div class="breadcrumb">
        ...
        <a href="{{ route('admin.agenda.show', $agenda) }}">{{ Str::limit($agenda->judul, 30) }}</a>
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
        <span>Edit</span>
    </div>
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Perbarui informasi agenda kegiatan sekolah</p>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <a href="{{ route('admin.agenda.show', $agenda) }}" class="btn btn-secondary">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-row cols-2" style="margin-bottom:16px;">
                            ...
                            <div class="form-group" style="margin-bottom:0">
                                <label>Tanggal Selesai <span class="hint">(opsional)</span></label>
                                <input type="date" name="tanggal_selesai"
                                    value="{{ old('tanggal_selesai', $agenda->tanggal_selesai ? \Carbon\Carbon::parse($agenda->tanggal_selesai)->format('Y-m-d') : '') }}"
                        ...
                        <div class="form-row cols-2">
                            <div class="form-group" style="margin-bottom:0">
                                ...
                                <input type="time" name="jam_mulai"
                                    value="{{ old('jam_mulai', $agenda->jam_mulai ? \Carbon\Carbon::parse($agenda->jam_mulai)->format('H:i') : '') }}"
                            ...
                            <div class="form-group" style="margin-bottom:0">
                                ...
                                <input type="time" name="jam_selesai"
                                    value="{{ old('jam_selesai', $agenda->jam_selesai ? \Carbon\Carbon::parse($agenda->jam_selesai)->format('H:i') : '') }}"
            ...
            <div>
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="tipe" class="form-control {{ $errors->has('tipe') ? 'is-invalid' : '' }}">
                                ...
                                @foreach($tipeList as $val => $label)
                                    <option value="{{ $val }}" {{ old('tipe', $agenda->tipe) == $val ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            @php $currentColor = old('warna', $agenda->warna ?? '#1f63db'); @endphp
                            <div class="color-picker-wrap">
                                <div class="color-preview">
                                    <div class="color-preview-inner" id="colorPreview" style="background:{{ $currentColor }}"></div>
                                    <input type="color" id="colorPicker" value="{{ $currentColor }}"
                                        style="position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%;">
                                </div>
                                <input type="text" name="warna" id="colorInput" value="{{ $currentColor }}"
                            ...
                            <div class="color-options">
                                @foreach(['#1f63db','#15803d','#dc2626','#a16207','#7c3aed','#0891b2','#be185d','#ea580c','#475569'] as $c)
                                    <span class="color-opt {{ $currentColor == $c ? 'active' : '' }}"
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <p class="card-title">Info Data</p>
                    </div>
                    <div class="card-body">
                        <div class="meta-info">
                            <div class="meta-row">
                                <span class="meta-key">ID Agenda</span>
                                <span class="meta-val">#{{ $agenda->id }}</span>
                            </div>
                            <div class="meta-row">
                                <span class="meta-key">Dibuat</span>
                                <span class="meta-val">{{ $agenda->created_at->translatedFormat('d M Y, H:i') }}</span>
                            </div>
                            <div class="meta-row">
                                <span class="meta-key">Diperbarui</span>
                                <span class="meta-val">{{ $agenda->updated_at->translatedFormat('d M Y, H:i') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-actions" style="flex-wrap:wrap;">
                    <form action="{{ route('admin.agenda.destroy', $agenda) }}" method="POST" id="deleteForm" style="margin:0">
                        @csrf @method('DELETE')
                        <button type="button" class="btn btn-danger-soft" onclick="confirmDelete()">
